...
const KNOCKOUT_ROUNDS = ["الملحق", "دور الستة عشر", "دور الثمانية", "نصف النهائي", "النهائي", "دور الـ 16", "دور الـ16", "دور 16", "دور الـ 8", "دور الـ8", "دور 8", "دور الـ 4", "دور الـ4", "دور 4"];
const MATHANI_BRACKET_ROUNDS = ["دور الـ 16", "دور الثمانية", "دور الـ 4", "النهائي"];
const toScore = (value: any) => { const n = Number(value); return Number.isFinite(n) ? n : 0; };
const normalizeRoundName = (round: any) => {
...
function buildStandings(matchRows: any[], allTeams: string[]) {
  const table = new Map<string, StandingRow>(); allTeams.forEach(team => table.set(normalizeTeamName(team), { team, played: 0, wins: 0, draws: 0, losses: 0, gf: 0, ga: 0, gd: 0, points: 0, rank: 0 }));
  const groupMatches = matchRows.filter((m) => {
    const stage = m.stage || getMathaniStage(m.round);
    return stage !== "knockout" && !isMathaniKnockoutRound(m.round);
  });
  groupMatches.forEach(match => { const hg = toScore(match.homeGoals); const ag = toScore(match.awayGoals); const hNorm = normalizeTeamName(match.home || match.teamA || ""); const aNorm = normalizeTeamName(match.away || match.teamB || ""); let home = table.get(hNorm); let away = table.get(aNorm); if (home) { home.played++; home.gf += hg; home.ga += ag; if (hg > ag) { home.wins++; home.points += 3; } else if (hg === ag) { home.draws++; home.points++; } else { home.losses++; } } if (away) { away.played++; away.gf += ag; away.ga += hg; if (ag > hg) { away.wins++; away.points += 3; } else if (ag === hg) { away.draws++; away.points++; } else { away.losses++; } } });
  const penalizedTeam = table.get(normalizeTeamName("17 فبراير")); if (penalizedTeam) { penalizedTeam.points -= 3; }
...
const getWinnerData = (t1: string, t2: string, round: string, labelId: string, allMatchesArr: any[]) => {
  ...
  if (!m) return { win: null, match: null }; let w = null;
  if (m.status === "انتهت") { const h = Number(m.homeGoals) || 0; const a = Number(m.awayGoals) || 0; if (h > a) w = m.teamA; else if (a > h) w = m.teamB; else { const hPen = (m.penaltiesHome || []).filter((p:any)=>p==='scored').length; const aPen = (m.penaltiesAway || []).filter((p:any)=>p==='scored').length; if (hPen > aPen) w = m.teamA; else if (aPen > hPen) w = m.teamB; } }
...
const DEFAULT_PLAYER_CARD_TOURNAMENTS = [
...
const PLAYER_CARD_FONT_FAMILY = "'Cairo', 'Tajawal', system-ui, sans-serif";
const PLAYER_CARD_ROLE_OPTIONS = [
  { value: "player", label: "لاعب" },
  { value: "manager", label: "مدير فني" },
];

const generatePlayerCardSerial = (prefix = "MTR") => {
...
export default function Page() {
  const [mainAppTab, setMainAppTab] = useState<'matrouh_cup' | 'elite_cup' | 'mathani_cup' | 'player_cards' | 'shop' | 'settings'>('matrouh_cup');
  useEffect(() => {
  ...
  const [selectedPlayerTournamentId, setSelectedPlayerTournamentId] = useState("");
  const [playerCardForm, setPlayerCardForm] = useState<any>({ fullName: "", role: "player", teamName: "", birthDate: "", nationalId: "", photoPreview: "", photoFile: null, photoUrl: "", cropX: 50, cropY: 50, zoom: 1, serialNumber: "" });
  const [lastPlayerCard, setLastPlayerCard] = useState<any | null>(null);
  const [isSavingPlayerCard, setIsSavingPlayerCard] = useState(false);
  ...
  useEffect(() => {
    ...
    const unsubPlayerTournaments = listenToCollection("player_registration_tournaments", (docs) => setPlayerCardTournaments(docs.sort((a:any,b:any) => (Number(a.sortOrder ?? 9999) - Number(b.sortOrder ?? 9999)) || String(a.name || "").localeCompare(String(b.name || ""), "ar"))));
    const unsubPlayerRegistrations = listenToCollection("player_registrations", (docs) => setPlayerRegistrations(docs.sort((a:any,b:any) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")))));
    ...
    return () => { unsubMatches(); unsubGoals(); unsubCards(); unsubArchivedCards(); unsubMedia(); unsubMotm(); unsubPreds(); unsubForms(); unsubRosters(); unsubPlayerTournaments(); unsubPlayerRegistrations(); unsubProducts(); unsubTicker(); unsubBanned(); unsubRestricted(); unsubMathaniGroups(); unsubTournamentLineup(); unsubRegMatrouh(); unsubRegElite(); clearInterval(clockTimer); };
  ...
  const validateRoster = () => {
     if(!rosterForm.teamName.trim()) return "يرجى كتابة اسم الفريق.";
     if(!rosterForm.managerName || !rosterForm.managerPhone) return "الرجاء إكمال بيانات مسئول الفريق (الاسم ورقم الهاتف)";
     const isTeamBanned = bannedEntities.some(b => b.type === 'team' && normalizeTeamName(b.name) === normalizeTeamName(rosterForm.teamName));
     if(isTeamBanned) return `عذراً، فريق "${rosterForm.teamName}" مستبعد ولا يمكنه المشاركة في البطولة. 🚫`;

     let restrictedCount = 0;
     for (let i=0; i<rosterForm.players.length; i++) {
        const p = rosterForm.players[i];
        if(!p.name.trim() || !p.number.trim()) return `الرجاء ملء بيانات جميع اللاعبين. اللاعب رقم ${i+1} بياناته ناقصة.`;
        if(!p.personalImageFile || !p.idImageFile) return `الرجاء إرفاق الصورة الشخصية وصورة البطاقة للاعب ${p.name || i+1}.`;
        const isPlayerBanned = bannedEntities.some(b => b.type === 'player' && normalizeTeamName(b.name) === normalizeTeamName(p.name));
        if(isPlayerBanned) return `عذراً، اللاعب "${p.name}" مستبعد ولا يمكن تسجيله. 🚫`;
        const isRestricted = restrictedPlayers.some(r => normalizeTeamName(r.name) === normalizeTeamName(p.name));
        if(isRestricted) restrictedCount++;
     }
     
     if (restrictedCount > 2) return `عذراً، لقد قمت بتسجيل ${restrictedCount} لاعبين من قائمة التقييد. الحد الأقصى المسموح به هو 2 لاعبين فقط في الفريق الواحد. ⚠️`;
     return null;
  };

  const submitFinalRoster = async () => {
    const errorMsg = validateRoster(); if(errorMsg) return alert(errorMsg);
    if(confirm("تنبيه هام: سيتم الآن رفع الصور وحفظ القائمة. قد تستغرق العملية دقيقة، يرجير عدم إغلاق الصفحة. هل أنت متأكد؟")) {
        setIsUploading(true);
        try {
            let suffix = "";
            if(mainAppTab === 'matrouh_cup') { const edSuffix = cupEdition === "edition_4" ? "_ed4" : ""; const tourSuffix = activeTournament === "juniors" ? "_juniors" : ""; suffix = edSuffix + tourSuffix; } else if(mainAppTab === 'elite_cup') { suffix = "_elite"; }
            
            const normalizedId = normalizeTeamName(rosterForm.teamName);
            const finalPlayers = await Promise.all(rosterForm.players.map(async (p:any, i:number) => {
                let pImg = ""; let idImg = "";
                if (p.personalImageFile) {
                    const imgRef = ref(storage, `rosters/${normalizedId}/player_${i}_personal_${Date.now()}`);
                    await uploadBytes(imgRef, p.personalImageFile);
                    pImg = await getDownloadURL(imgRef);
                }
                if (p.idImageFile) {
                    const imgRef = ref(storage, `rosters/${normalizedId}/player_${i}_id_${Date.now()}`);
                    await uploadBytes(imgRef, p.idImageFile);
                    idImg = await getDownloadURL(imgRef);
                }
                return { name: p.name, number: p.number, personalImage: pImg, idImage: idImg };
            }));

            await setDoc(doc(db, `team_rosters${suffix}`, normalizedId), { id: normalizedId, teamName: rosterForm.teamName.trim(), managerName: rosterForm.managerName, managerPhone: rosterForm.managerPhone, logoUrl: rosterForm.logoUrl, players: finalPlayers, isSubmitted: true, updatedAt: new Date().toISOString() }, { merge: true });
            alert("✅ تم رفع الصور واعتماد قائمة الفريق بنجاح!"); setUnlockedRoster(null); setRosterAccessPassword(""); setRosterViewMode('list'); setShowPaymentForm(true);
        } catch(e) { alert("حدث خطأ أثناء رفع الصور، يرجى المحاولة مرة أخرى."); }
  ...
  const liveMatchesList = matches.filter(m => { if (m.isLive === true || m.status === "live" || m.status === "مباشر" || m.status === "شغال الآن") return true; if (m.date === todayStr && m.time) { const now = time || new Date(); const [hours, minutes] = m.time.split(':').map(Number); const matchTime = new Date(); matchTime.setHours(hours, minutes, 0, 0); const diffMins = (now.getTime() - matchTime.getTime()) / 60000; if (diffMins >= -30 && diffMins <= 180 && m.status !== "انتهت") return true; } return false; });
  const liveMatches = sortMatchesAsc(liveMatchesList); const liveMatchIds = new Set(liveMatchesList.map(m => m.id));
  ...
  const youthTree = useMemo(() => {
    ...
    const p97 = getWinnerData("اسماك باسط العوامي", "اصدقاء عز بوالمجدوبة", "الملحق", "م 97", matches); const p98 = getWinnerData("السلوم", "اصدقاء عيسي المغواري", "الملحق", "م 98", matches); const p100 = getWinnerData("اصدقاء قسم الله", "اصدقاء سلامة بدر", "الملحق", "م 100", matches); const p101 = getWinnerData("ايس كريم الملكة", "غوط رباح", "الملحق", "م 101", matches); const p102 = getWinnerData("محاربي الصحراء", "اصدقاء خالد", "الملحق", "م 102", matches);
    const r1 = getWinnerData(getT(1), "الفائز من م 104", "دور الستة عشر", "م 1", matches); const r2 = getWinnerData(getT(8), p97.win || "الفائز من م 97", "دور الستة عشر", "م 2", matches); const r3 = getWinnerData("غوط رباح", "القدس", "دور الستة عشر", "م 3", matches); const r4 = getWinnerData(getT(5), p100.win || "الفائز من م 100", "دور الستة عشر", "م 4", matches); const r5 = getWinnerData("سامي سعيد", "شباب القناشات", "دور الستة عشر", "م 5", matches); const r6 = getWinnerData(getT(7), p98.win || "الفائز من م 98", "دور الستة عشر", "م 6", matches); const r7 = getWinnerData("اصدقاء خالد", "براني", "دور الستة عشر", "م 7", matches);
    const q1 = getWinnerData(r1.win || "الفائز (م 1)", r2.win || "الفائز (م 2)", "دور الثمانية", "مربع 1", matches); const q2 = getWinnerData(r3.win || "الفائز (م 3)", r4.win || "الفائز (م 4)", "دور الثمانية", "مربع 2", matches); const q3 = getWinnerData(r5.win || "الفائز (م 5)", r6.win || "الفائز (م 6)", "دور الثمانية", "مربع 3", matches);
    const s1 = getWinnerData(q1.win || "الفائز مربع 1", q2.win || "الفائز مربع 2", "نصف النهائي", "نصف 1", matches); const s2 = getWinnerData(q3.win || "الفائز مربع 3", "الفائز مربع 4", "نصف النهائي", "نصف 2", matches);
    const f1 = getWinnerData(s1.win || "الطرف الأول", s2.win || "الطرف الثاني", "النهائي", "النهائي", matches);
    return { p97, p98, p100, p101, p102, r1, r2, r3, r4, r5, r6, r7, q1, q2, q3, s1, s2, f1, getT };
  ...
  const juniorsTree = useMemo(() => {
    ...
    const q1 = getWinnerData(ja1, jb4, "دور الثمانية", "مربع 1", matches); const q2 = getWinnerData(jb2, ja3, "دور الثمانية", "مربع 2", matches); const q3 = getWinnerData(jb1, ja4, "دور الثمانية", "مربع 3", matches); const q4 = getWinnerData(ja2, jb3, "دور الثمانية", "مربع 4", matches);
    const s1 = getWinnerData(q1.win || "الفائز (مربع 1)", q2.win || "الفائز (مربع 2)", "نصف النهائي", "نصف 1", matches); const s2 = getWinnerData(q3.win || "الفائز (مربع 3)", q4.win || "الفائز (مربع 4)", "نصف النهائي", "نصف 2", matches);
    const f1 = getWinnerData(s1.win || "الطرف الأول", s2.win || "الطرف الثاني", "النهائي", "النهائي", matches);
  ...
  const scorers = useMemo(() => {
    ...
    goalEvents.forEach(e => { const playerStr = normalizeTeamName(e.player || ""); const playerOriginal = String(e.player || "").trim(); const teamStr = String(e.team || "").trim(); if(!playerOriginal) return; const key = `${playerStr}__${normalizeTeamName(teamStr)}`; if (!map.has(key)) { map.set(key, { name: playerOriginal, player: playerOriginal, team: teamStr, goals: 0, imageUrl: e.imageUrl || "", rating: e.rating || 99, pac: e.pac || 99, sho: e.sho || 99, pas: e.pas || 99, dri: e.dri || 99, def: e.def || 99, phy: e.phy || 99 }); } map.get(key)!.goals += Number(e.goals) || 1; if (e.imageUrl && !map.get(key)!.imageUrl) map.get(key)!.imageUrl = e.imageUrl; if (e.rating) Object.assign(map.get(key)!, { rating: e.rating, pac: e.pac, sho: e.sho, pas: e.pas, dri: e.dri, def: e.def, phy: e.phy }); });
    return Array.from(map.values()).sort((a, b) => b.goals - a.goals);
  }, [goalEvents]);

  const filteredScorers = useMemo(() => { if (!searchScorers) return scorers; const term = searchScorers.toLowerCase().trim(); return scorers.filter(s => String(s.player || "").toLowerCase().includes(term) || String(s.team || "").toLowerCase().includes(term)); }, [scorers, searchScorers]);
  const topMotmPlayers = useMemo(() => { const counts: Record<string, any> = {}; motmList.forEach(m => { const p = String(m.player || "").trim(); if(!p) return; if(!counts[p]) counts[p] = { name: p, team: m.team, count: 0, imageUrl: m.imageUrl, sponsorLogo: m.sponsorLogo }; counts[p].count++; }); return Object.values(counts).sort((a: any, b: any) => b.count - a.count).slice(0, 3); }, [motmList]);
  const fantasyLeaderboard = useMemo(() => { const users: Record<string, any> = {}; predictionsList.forEach(p => { const phone = p.phone; if (!users[phone]) users[phone] = { name: p.name, points: 0, correctScore: 0, correctWinner: 0 }; const match = finishedMatches.find(m => m.id === p.matchId || m.teamA + " vs " + m.teamB === p.matchName); if (match) { const pHome = Number(p.homeScore); const pAway = Number(p.awayScore); const mHome = Number(match.homeGoals); const mAway = Number(match.awayGoals); if (pHome === mHome && pAway === mAway) { users[phone].points += 3; users[phone].correctScore++; } else if ((pHome > pAway && mHome > mAway) || (pHome < pAway && mHome < mAway) || (pHome === pAway && mHome === mAway)) { users[phone].points += 1; users[phone].correctWinner++; } } }); return Object.values(users).sort((a,b) => b.points - a.points || b.correctScore - a.correctScore).slice(0, 10); }, [predictionsList, finishedMatches]);

  const statsData = useMemo(() => {
    const completedMatchesAllRounds = matches.filter(m => m.status === "انتهت" || (!liveMatchIds.has(m.id) && m.date < todayStr)); const totalMatches = completedMatchesAllRounds.length; let totalGoals = 0, draws00 = 0, drawsPositive = 0; const teamStats = new Map<string, {team: string, gf: number, ga: number}>(); const allTeams = activeTournament === 'youth' ? CLEANED_TEAM_NAMES : [...JUNIORS_GROUP_A, ...JUNIORS_GROUP_B]; allTeams.forEach(t => teamStats.set(normalizeTeamName(t), { team: t, gf: 0, ga: 0 }));
    completedMatchesAllRounds.forEach(m => { const hg = Number(m.homeGoals) || 0; const ag = Number(m.awayGoals) || 0; totalGoals += (hg + ag); if (hg === ag) { if (hg === 0) draws00++; else drawsPositive++; } const hNorm = normalizeTeamName(m.teamA); const aNorm = normalizeTeamName(m.teamB); if (teamStats.has(hNorm)) { teamStats.get(hNorm)!.gf += hg; teamStats.get(hNorm)!.ga += ag; } if (teamStats.has(aNorm)) { teamStats.get(aNorm)!.gf += ag; teamStats.get(aNorm)!.ga += hg; } });
    const allCardEvents = [...cardEvents, ...archivedCards]; const totalYellow = allCardEvents.reduce((acc, curr) => acc + (Number(curr.yellow) || 0), 0); const totalRed = allCardEvents.reduce((acc, curr) => acc + (Number(curr.red) || 0), 0); const activeTeams = Array.from(teamStats.values()).filter(t => t.gf > 0 || t.ga > 0 || totalMatches > 0); const sortedByAttack = [...activeTeams].sort((a, b) => b.gf - a.gf); const sortedByDef = [...activeTeams].sort((a, b) => a.ga - b.ga); 
    return { totalMatches, totalGoals, draws00, drawsPositive, totalYellow, totalRed, bestAttack: sortedByAttack[0] || { team: "—", gf: 0 }, worstAttack: sortedByAttack[sortedByAttack.length - 1] || { team: "—", gf: 0 }, bestDefense: sortedByDef[0] || { team: "—", ga: 0 }, worstDefense: sortedByDef[sortedByDef.length - 1] || { team: "—", ga: 0 }, topScorer: scorers[0], goalsPerMatch: totalMatches > 0 ? (totalGoals / totalMatches).toFixed(2) : "0", draws00Percent: totalMatches > 0 ? Math.round((draws00 / totalMatches) * 100) : 0, drawsPosPercent: totalMatches > 0 ? Math.round((drawsPositive / totalMatches) * 100) : 0, yellowPerMatch: totalMatches > 0 ? (totalYellow / totalMatches).toFixed(2) : "0", redPerMatch: totalMatches > 0 ? (totalRed / totalMatches).toFixed(2) : "0" };
  ...
  const cardsList = useMemo(() => { const map = new Map<string, any>(); activeCardsSource.forEach(e => { const playerStr = normalizeTeamName(e.player || ""); const playerOriginal = String(e.player || "").trim(); if(!playerOriginal) return; const key = `${playerStr}__${normalizeTeamName(e.team)}`; if (!map.has(key)) map.set(key, { player: playerOriginal, team: e.team, yellow: 0, red: 0 }); const item = map.get(key)!; item.yellow += Number(e.yellow) || 0; item.red += Number(e.red) || 0; }); return Array.from(map.values()).map(row => ({ ...row, status: row.red > 0 ? "طرد" : row.yellow >= 3 ? "إيقاف" : "متاح" })).sort((a, b) => b.red - a.red || b.yellow - a.yellow); }, [activeCardsSource]);
  const filteredCardsList = useMemo(() => { if (!searchCards) return cardsList.filter(c => c.yellow > 0 || c.red > 0); const term = searchCards.toLowerCase().trim(); return cardsList.filter(c => (c.yellow > 0 || c.red > 0) && (String(c.player || "").toLowerCase().includes(term) || String(c.team || "").toLowerCase().includes(term))); }, [cardsList, searchCards]);
  ...
  const handlePlayerCardPhotoUpload = (file?: File) => {
    if (!file) return;
    if (!file.type.startsWith("image/")) return alert("يرجى رفع صورة فقط");
    const previewUrl = URL.createObjectURL(file);
    setPlayerCardForm((p:any) => ({ ...p, photoPreview: previewUrl, photoFile: file, photoUrl: p.photoUrl || "", cropX: 50, cropY: 50, zoom: 1 }));
  };

  const playerCardRoleLabel = (value?: string) => PLAYER_CARD_ROLE_OPTIONS.find((r) => r.value === value)?.label || "لاعب";

  const availablePlayerCardTournaments = useMemo(() => {
    const active = (playerCardTournaments || []).filter((t:any) => t.isActive !== false);
    return active.length ? active : DEFAULT_PLAYER_CARD_TOURNAMENTS;
  }, [playerCardTournaments]);

  useEffect(() => {
    if (!availablePlayerCardTournaments.length) return;
    if (!selectedPlayerTournamentId || !availablePlayerCardTournaments.some((t:any) => t.id === selectedPlayerTournamentId)) {
      setSelectedPlayerTournamentId(availablePlayerCardTournaments[0].id);
    }
  }, [availablePlayerCardTournaments, selectedPlayerTournamentId]);

  const selectedPlayerCardTournament = useMemo(() => {
    return availablePlayerCardTournaments.find((t:any) => t.id === selectedPlayerTournamentId) || availablePlayerCardTournaments[0] || DEFAULT_PLAYER_CARD_TOURNAMENTS[0];
  }, [availablePlayerCardTournaments, selectedPlayerTournamentId]);

  const currentPlayerCardPreview = useMemo(() => {
    const registrationDate = new Date().toISOString().slice(0, 10);
    return {
      ...playerCardForm,
      fullName: String(playerCardForm.fullName || "").trim(),
      playerName: String(playerCardForm.fullName || "").trim(),
      roleLabel: playerCardRoleLabel(playerCardForm.role),
      teamName: String(playerCardForm.teamName || "").trim() || "لاعب حر",
      registrationDate,
      tournamentId: selectedPlayerCardTournament?.id || "",
      tournamentName: selectedPlayerCardTournament?.name || "بطولة رياضية",
      tournamentLogoUrl: selectedPlayerCardTournament?.logoUrl || "/logo.png",
      brandLogoUrl: PLAYER_CARD_BRAND_LOGO,
      signatureImageUrl: PLAYER_CARD_SIGNATURE_IMAGE,
      cropX: Number(playerCardForm.cropX ?? 50),
      cropY: Number(playerCardForm.cropY ?? 50),
      zoom: Number(playerCardForm.zoom ?? 1),
      photoUrl: playerCardForm.photoPreview || playerCardForm.photoUrl || "",
      serialNumber: playerCardForm.serialNumber || `${String(selectedPlayerCardTournament?.id || "MTR").replace(/[^A-Za-z0-9]/g, "").slice(0, 4).toUpperCase() || "MTR"}-${new Date().getFullYear()}-PREVIEW`,
      qrPayload: JSON.stringify({ serial: playerCardForm.serialNumber || "PREVIEW", name: String(playerCardForm.fullName || "").trim(), role: playerCardRoleLabel(playerCardForm.role), tournament: selectedPlayerCardTournament?.name || "بطولة رياضية" }),
    };
  }, [playerCardForm, selectedPlayerCardTournament]);

  const savePlayerCardRegistration = async () => {
    const fullName = String(playerCardForm.fullName || "").trim();
    const birthDate = String(playerCardForm.birthDate || "").trim();
    const nationalId = String(playerCardForm.nationalId || "").trim();
    const teamName = String(playerCardForm.teamName || "").trim() || "لاعب حر";
    if (!fullName) return alert("اكتب الاسم");
    if (!birthDate) return alert("اكتب تاريخ الميلاد");
    if (!nationalId) return alert("اكتب الرقم القومي");
    if (nationalId.replace(/\D/g, "").length < 10) return alert("الرقم القومي غير مكتمل");
    setIsSavingPlayerCard(true);
    try {
      const registrationDate = new Date().toISOString().slice(0, 10);
      let uploadedPhotoUrl = String(playerCardForm.photoUrl || "").trim();
      const serialNumber = playerCardForm.serialNumber || generatePlayerCardSerial(selectedPlayerCardTournament?.id || "MTR");
      if (playerCardForm.photoFile) {
        const safeName = `${Date.now()}_${fullName.replace(/[^\w؀-ۿ-]+/g, "_")}.jpg`;
        const photoRef = ref(storage, `player_registration_photos/${safeName}`);
        await uploadBytes(photoRef, playerCardForm.photoFile);
        uploadedPhotoUrl = await getDownloadURL(photoRef);
      }
      const payload = {
        playerName: fullName,
        fullName,
        role: playerCardForm.role || "player",
        roleLabel: playerCardRoleLabel(playerCardForm.role),
        teamName,
        birthDate,
        nationalId,
        ...
        zoom: Number(playerCardForm.zoom ?? 1),
        serialNumber,
        qrPayload: JSON.stringify({ serial: serialNumber, name: fullName, role: playerCardRoleLabel(playerCardForm.role), tournament: selectedPlayerCardTournament?.name || "بطولة رياضية" }),
        status: "new",
        createdAt: new Date().toISOString(),
      };
      const docRef = await addDoc(collection(db, "player_registrations"), payload);
      setLastPlayerCard({ id: docRef.id, ...payload });
      setPlayerCardForm((p:any) => ({ ...p, serialNumber, photoUrl: uploadedPhotoUrl || p.photoPreview || "", photoFile: null }));
      alert("تم تسجيل البيانات وتجهيز الكارت للطباعة ✅");
    } catch (e:any) {
      console.error("Player registration error", e);
      alert(e?.message || "حدث خطأ أثناء تسجيل اللاعب");
    }
    setIsSavingPlayerCard(false);
  };
  ...
  const visibleTabs = allHomeTabs.filter(tab => {
     if (cupEdition === 'edition_3') { return !['live', 'today', 'tomorrow'].includes(tab.key); }
     return tab.key !== 'champion';
  });
    ...
    <div dir="rtl" className="min-h-screen bg-[#0a1428] text-white relative pb-20 font-sans">
      <div className="mx-auto max-w-7xl p-3 sm:p-4 md:p-6 lg:p-8">
        ...
        {time && (
          <div className="mb-4 rounded-3xl border border-white/10 bg-gradient-to-r from-[#1e2a4a] to-[#13213a] p-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl relative overflow-hidden">
             ...
             <div className="flex items-center gap-3 relative z-10">
               <div className="bg-yellow-400/20 p-2.5 rounded-xl border border-yellow-400/30 shadow-inner"><Calendar className="text-yellow-400 h-5 w-5" /></div>
               <span className="text-white font-bold text-sm sm:text-base tracking-wide">{formattedDate}</span>
             </div>
             <div className="flex items-center gap-4 relative z-10">
               <span className="text-2xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-l from-yellow-200 to-yellow-500 tracking-widest drop-shadow-sm" dir="ltr">{formattedTime}</span>
               <div className="bg-cyan-500/20 p-2.5 rounded-xl border border-cyan-500/30 shadow-inner"><Clock className="text-cyan-400 h-5 w-5 animate-pulse" /></div>
             </div>
          </div>
        )}
        ...
        <div className="mb-6 rounded-3xl border border-yellow-400/40 bg-gradient-to-br from-[#1e2a4a] to-[#13213a] p-6 text-center shadow-2xl relative overflow-hidden">
          ...
          <div className="flex justify-center mb-6 relative z-10"><img src="/logo.png" alt="شعار مطروح الرياضية" className="h-20 sm:h-28 lg:h-32 w-auto max-w-[90%] object-contain drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]" /></div>
        ...
        <div className="flex justify-center mb-6 overflow-x-auto touch-pan-x custom-scrollbar pb-2 px-2 w-full">
          <div className="bg-[#13213a] p-2 rounded-2xl border border-yellow-400/30 inline-flex shadow-xl gap-2 w-max min-w-full sm:min-w-0 sm:w-auto">
            ...
            <button onClick={() => setMainAppTab('player_cards')} className={`flex-1 py-3 px-4 sm:px-6 rounded-xl text-sm sm:text-lg font-black transition-all ${mainAppTab === 'player_cards' ? 'bg-cyan-500 text-black shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>🪪 تسجيل اللاعبين</button>
        ...
        {mainAppTab === 'mathani_cup' && (
          <div className="space-y-6 animate-in fade-in duration-500 max-w-6xl mx-auto">
            ...
            <div className="flex justify-center gap-2 mb-6 flex-wrap">
              {[ {id: 'standings', name: 'جدول الترتيب'}, {id: 'bracket', name: 'الأدوار الإقصائية'}, {id: 'upcoming', name: 'المباريات القادمة'}, {id: 'results', name: 'نتائج المباريات'}, {id: 'stats', name: 'الإحصائيات'}, {id: 'scorers', name: 'الهدافون'}, {id: 'banned', name: 'الإنذارات والموقوفين'} ].map(t => (
            ...
            {/* TAB: KNOCKOUT BRACKET (MATHANI) */}
            {activeMathaniTab === 'bracket' && (
              <div className="space-y-8 animate-in fade-in duration-500">
                {(() => {
                  const knockoutMatches = sortMatchesAsc(
                    matches.filter((m) => {
                      const stage = m.stage || getMathaniStage(m.round);
                      return stage === "knockout" || isMathaniKnockoutRound(m.round);
                    })
                  ...
                  if (knockoutMatches.length === 0) {
                    return (
                      <div className="text-center text-emerald-400/70 font-bold py-16 bg-[#13213a] rounded-3xl border border-emerald-500/20 shadow-inner flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-xl">لم تتم إضافة مباريات الأدوار الإقصائية بعد.</p>
                        <p className="text-sm text-gray-400 max-w-2xl">أضف مباريات دور الـ16 من لوحة التحكم بأكواد R16-1 إلى R16-8، وسيظهر مسار الصعود هنا تلقائيًا.</p>
                      </div>
                    );
                  }

                  return MATHANI_BRACKET_ROUNDS.map((roundName) => {
                    const roundMatches = knockoutMatches.filter((m) => normalizeRoundName(m.round) === roundName);
                    if (roundMatches.length === 0) return null;

                    return (
                      <Card key={roundName} className="bg-[#13213a] border border-emerald-500/30 shadow-xl overflow-hidden">
                        <CardHeader className="border-b border-white/10 bg-emerald-500/10">
                          <CardTitle className="text-emerald-300 font-black flex items-center gap-2">
                            <Trophy className="w-5 h-5" /> {roundName}
                          </CardTitle>
                        </CardHeader>
                        ...
                        <CardContent className="p-4 grid gap-4 md:grid-cols-2">
                          {roundMatches.map((match) => {
                            ...
                            return (
                              <div key={match.id} className="bg-[#1e2a4a] rounded-2xl border border-white/10 p-4 hover:border-emerald-500/50 transition-all shadow-lg">
                                <div className="flex items-center justify-between gap-2 mb-4">
                                  <Badge className="bg-emerald-500 text-white font-black border-none" dir="ltr">
                                    {match.matchLabel || roundName}
                                  </Badge>
                                  <span className="text-xs text-gray-400 font-bold">
                                    {match.date || "غير محدد"} {match.time ? `• ${formatTime12(match.time)}` : ""}
                                  </span>
                                </div>

                                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-white font-black">
                                  <div className={`text-center leading-tight ${winner === match.teamA ? "text-yellow-300 scale-105" : ""}`}>
                                  ...
                                  <div className={`text-center leading-tight ${winner === match.teamB ? "text-yellow-300 scale-105" : ""}`}>
                                    {match.teamB || "لم يتحدد"}
                                  </div>
                                </div>

                                {winner ? (
                                  <div className="mt-4 text-center text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 rounded-xl py-2 font-black">
                                    صعد: {winner}
                                  </div>
                                ) : isEnded ? (
                                  <div className="mt-4 text-center text-yellow-300 bg-yellow-500/10 border border-yellow-500/30 rounded-xl py-2 font-bold text-sm">
            ...
            {activeMathaniTab === 'results' && (
               <div className="space-y-8">
                  {["دور المجموعات", "دور الـ 16", "دور الثمانية", "دور الـ 4", "النهائي"].map(roundName => {
                     const roundMatches = finishedMatches.filter((m) => roundName === "دور المجموعات" ? !isMathaniKnockoutRound(m.round) && (m.stage || "group") !== "knockout" : normalizeRoundName(m.round) === roundName);
                     if (roundMatches.length === 0) return null;
                     return (
        ...
        {mainAppTab === 'matrouh_cup' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-center mb-4 px-2">
              <div className="bg-[#1e2a4a] p-1.5 rounded-full border border-white/10 flex shadow-inner gap-1 w-full max-w-md">
            ...
            <div className="flex justify-center mb-8 px-2">
              <div className="bg-[#13213a] p-2 rounded-full border border-yellow-400/30 flex shadow-xl gap-2 w-full max-w-md">
                <button onClick={() => { setActiveTournament('youth'); setActiveTab(cupEdition === 'edition_3' ? 'champion' : 'standings'); }} className={`flex-1 py-3 rounded-full text-base sm:text-xl font-black transition-all ${activeTournament === 'youth' ? 'bg-yellow-400 text-black shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>🏆 الشباب</button>
                <button onClick={() => { setActiveTournament('juniors'); setActiveTab(cupEdition === 'edition_3' ? 'champion' : 'standings'); }} className={`flex-1 py-3 rounded-full text-base sm:text-xl font-black transition-all ${activeTournament === 'juniors' ? 'bg-cyan-500 text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>🏅 الناشئين</button>
              </div>
            </div>

            <div className="flex overflow-x-auto w-full touch-pan-x snap-x snap-mandatory gap-2 pb-4 mb-8 custom-scrollbar">
              {visibleTabs.map(tab => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`shrink-0 snap-center px-4 sm:px-5 py-2.5 sm:py-3.5 rounded-2xl font-bold text-sm sm:text-base transition-all border ${activeTab === tab.key ? (tab.extraClass || "bg-yellow-400 text-black border-yellow-400 shadow-lg scale-105") : "bg-[#1e2a4a] text-white border-yellow-400/30 hover:bg-[#25345a]"}`}>
                  <span className={`text-lg`}>{tab.icon}</span> <span className="ml-1">{tab.label}</span>
                </button>
              ))}
            </div>
            ...
            {activeTab === "champion" && cupEdition === 'edition_3' && (
              <div className="space-y-8 animate-in zoom-in duration-500 max-w-4xl mx-auto mt-4">
                 <Card className="bg-gradient-to-b from-[#facc15]/20 to-[#13213a] border-2 border-yellow-400 rounded-[2rem] overflow-hidden shadow-[0_0_40px_rgba(250,204,21,0.3)] text-center relative p-1">
                   <CardContent className="p-8 sm:p-14 relative z-10 flex flex-col items-center bg-gradient-to-b from-[#13213a]/80 to-[#0a1428] rounded-[1.8rem]">
                     <div className="w-28 h-28 sm:w-36 sm:h-36 bg-gradient-to-tr from-yellow-300 to-yellow-600 rounded-full flex items-center justify-center mb-6 shadow-2xl border-4 border-[#0a1428]">
                        <Trophy className="h-14 w-14 sm:h-16 sm:w-16 text-[#0a1428]" />
                     </div>
                     <Badge className="bg-yellow-400 text-black font-black px-6 py-2 text-sm sm:text-lg mb-6 border-2 border-[#0a1428]">بطل كأس مطروح (النسخة الثالثة)</Badge>
                     <h2 className="text-6xl sm:text-8xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-200 to-yellow-500 mb-8">{activeTournament === 'youth' ? 'وادي ماجد' : 'وادي الرمل'}</h2>
                     <div className="w-full aspect-video bg-[#0a1428] rounded-2xl border-4 border-yellow-400/50 overflow-hidden shadow-2xl">
                        <img src={activeTournament === 'youth' ? '/champion.jpg' : '/champion_juniors.jpg'} alt="صورة تتويج البطل" className="w-full h-full object-cover" />
                     </div>
                   </CardContent>
                 </Card>
              </div>
                ...
                <div className="flex justify-center mb-6">
                   <div className="bg-[#13213a] p-1.5 rounded-2xl border border-white/10 inline-flex shadow-lg gap-1 w-full max-w-md">
                     ...
                     {cupEdition === 'edition_4' && (
                        <button onClick={() => { setRosterViewMode('register'); setUnlockedRoster(null); setShowPaymentForm(true); }} className={`flex-1 py-3 rounded-xl text-base font-bold transition-all ${rosterViewMode === 'register' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-400 hover:text-white'}`}><Lock className="inline-block mr-1 h-5 w-5" /> تسجيل فريق جديد</button>
                     )}
                   </div>
                </div>
            ...
            {activeTab === "fantasy" && (
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500">
                  <Card className="lg:col-span-2 bg-[#13213a] border border-emerald-500/30 rounded-3xl p-4 sm:p-6 shadow-xl">
                     <CardHeader><CardTitle className="text-emerald-400 font-black flex items-center gap-2"><Zap/> توقع واكسب هدايا وجوائز البطولة!</CardTitle></CardHeader>
                     <CardContent className="space-y-6">
                        {liveMatches.length > 0 ? liveMatches.map(match => {
                           const hasPred = predictedMatches[match.id];
                           return (
                             <div key={match.id} className="bg-[#1e2a4a] border border-white/5 p-4 rounded-2xl">
                                <div className="text-center text-xs text-cyan-300 font-bold mb-3">{match.round} • توقع النتيجة بدقة قبل انطلاق المباراة</div>
                                <div className="flex items-center justify-center gap-3 text-center mb-4"><span className="font-black text-white text-sm sm:text-base flex-1">{match.teamA}</span><span className="text-xs text-gray-400 px-3 py-1 bg-[#0a1428] rounded-full border border-white/5">VS</span><span className="font-black text-white text-sm sm:text-base flex-1">{match.teamB}</span></div>
                                {hasPred ? (
                                   <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-center font-bold py-2 rounded-xl text-sm">✓ تم تسجيل توقعك للمباراة بنجاح، حظ سعيد!</div>
                                ) : (
                                   <div className="space-y-3 max-w-sm mx-auto">
                                      <div className="flex gap-2 justify-center items-center" dir="ltr">
                                         <Input type="number" placeholder="0" onChange={e => setPredForms(p => ({...p, [match.id]: {...p[match.id], homeScore: e.target.value}}))} className="bg-[#0a1428] border-none text-white text-center font-black text-xl h-11 w-16" />
                                         <span className="text-white font-bold">:</span>
                                         <Input type="number" placeholder="0" onChange={e => setPredForms(p => ({...p, [match.id]: {...p[match.id], awayScore: e.target.value}}))} className="bg-[#0a1428] border-none text-white text-center font-black text-xl h-11 w-16" />
                                      </div>
                                      <Input placeholder="اسمك بالكامل" onChange={e => setPredForms(p => ({...p, [match.id]: {...p[match.id], name: e.target.value}}))} className="bg-[#0a1428] border-none text-white h-10 text-sm font-bold" />
                                      <Input placeholder="رقم الهاتف للتواصل" type="tel" onChange={e => setPredForms(p => ({...p, [match.id]: {...p[match.id], phone: e.target.value}}))} className="bg-[#0a1428] border-none text-white h-10 text-sm font-bold text-right" />
                                      <Button onClick={() => submitPrediction(match.id, `${match.teamA} vs ${match.teamB}`)} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold h-11 text-sm rounded-xl">إرسال التوقع 🎁</Button>
                                   </div>
                                )}
                             </div>
                           );
                        }) : <p className="text-center text-gray-400 font-bold py-6">لا توجد مباريات قادمة متاحة للتوقع حالياً.</p>}
                     </CardContent>
                  </Card>
              ...
              <div className="space-y-8 animate-in fade-in duration-500">
                {activeTournament === 'juniors' ? (
                  <div className="grid md:grid-cols-2 gap-8">
                    {[ { title: "المجموعة الأولى (أ) - ناشئين", data: standingsJunA }, { title: "المجموعة الثانية (ب) - ناشئين", data: standingsJunB } ].map(group => (
                  ...
                  <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] shadow-xl overflow-hidden">
                     <CardHeader className="flex flex-row items-center justify-between border-b border-yellow-400/20 pb-4"><CardTitle className="text-yellow-300 flex items-center gap-3"><Trophy className="h-7 w-7" /> جدول الترتيب العام للشباب</CardTitle><Button size="sm" onClick={() => setIsTableExpanded(!isTableExpanded)} className="bg-yellow-400 text-black hover:bg-yellow-500 font-bold flex items-center gap-2">{isTableExpanded ? <Minimize className="h-4 w-4" /> : <Maximize className="h-4 w-4" />} {isTableExpanded ? "تصغير الشاشة" : "عرض الشاشة بالعرض"}</Button></CardHeader>
                     <CardContent className="p-0">
                   ...
                   {finishedMatches.filter(m => !search || String(m.teamA || "").includes(search.trim()) || String(m.teamB || "").includes(search.trim())).map(match => (
                     <div key={match.id} className={`bg-[#1e2a4a] p-6 rounded-3xl border border-white/5 text-center transition-all hover:scale-[1.01] ${activeTournament === 'juniors' ? 'hover:border-cyan-400/50' : 'hover:border-yellow-400/50'}`}>
                       ...
                       <div className="flex items-center justify-center gap-2 sm:gap-6 mt-4">
                         <TeamMatchDisplay teamName={match.teamA} logoUrl={match.teamALogo} />
                         <div className="bg-[#0a1428] rounded-xl py-2 px-4 border border-white/5 text-yellow-400 shadow-inner shrink-0">{renderMatchScore(match)}</div>
                         <TeamMatchDisplay teamName={match.teamB} logoUrl={match.teamBLogo} />
                       </div>
                     </div>
                   ))}
                   {finishedMatches.length === 0 && <div className="col-span-full text-center text-gray-400 font-bold py-10">لا توجد مباريات مسجلة في قاعدة البيانات لهذه الفئة حالياً.</div>}
                 </CardContent>
               </Card>
            )}

            {/* TAB: TODAY & TOMORROW */}
            {activeTab === "today" && ( <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] p-8 text-center font-bold text-gray-400">هذه البطولة مؤرشفة، لا توجد مباريات جارية اليوم 📅</Card> )}
            {activeTab === "tomorrow" && ( <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] p-8 text-center font-bold text-gray-400">لا توجد مباريات مسجلة لليوم التالي 📆</Card> )}
            
            {/* TAB: SCORERS */}
            {activeTab === "scorers" && (
               <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] p-4 sm:p-6 animate-in fade-in duration-500">
                  <CardHeader className="flex flex-col sm:flex-row justify-between items-center gap-4 pb-4 border-b border-white/10 mb-4"><div><CardTitle className="text-yellow-300 font-black">قائمة هدافي البطولة الرسمية</CardTitle></div><div className="relative w-full sm:w-64"><Search className="absolute right-3 top-3 h-4 w-4 text-cyan-300" /><Input value={searchScorers} onChange={e => setSearchScorers(e.target.value)} placeholder="بحث عن لاعب..." className="pr-10 bg-[#1e2a4a] border-yellow-400/50 text-white rounded-xl" /></div></CardHeader>
                  <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                     {filteredScorers.map((s, i) => (
                        <div key={i} className="bg-[#1e2a4a] p-4 rounded-2xl border border-white/5 flex items-center justify-between shadow-md"><div className="flex items-center gap-3"><Badge className={i===0?"bg-yellow-400 text-black":i===1?"bg-gray-300 text-black shadow-md":"bg-gray-800"}>{i+1}</Badge><div><h4 className="font-black text-white text-base leading-tight">{s.player}</h4><span className="text-gray-400 text-xs font-bold mt-1 inline-block">{s.team}</span></div></div><Badge className="bg-cyan-500/20 text-cyan-400 text-lg font-black border border-cyan-500/30 px-3.5 py-1" dir="ltr">{s.goals} ⚽</Badge></div>
                     ))}
                  </CardContent>
               </Card>
            )}
            ...
            {activeTab === "cards" && (
               <Card className="rounded-3xl border border-red-500/30 bg-[#13213a] p-4 sm:p-6 animate-in fade-in duration-500">
                  <CardHeader className="flex flex-col sm:flex-row justify-between items-center gap-4 pb-4 border-b border-white/10 mb-4"><div className="flex items-center gap-3"><CardTitle className="text-red-400 font-black">سجل العقوبات والكروت</CardTitle><div className="flex gap-1 border border-white/10 rounded-xl p-1 bg-[#0a1428]"><button onClick={() => setShowArchivedCards(false)} className={`text-xs px-3 py-1 rounded-lg font-bold ${!showArchivedCards ? 'bg-red-500 text-white' : 'text-gray-400'}`}>الحالية</button><button onClick={() => setShowArchivedCards(true)} className={`text-xs px-3 py-1 rounded-lg font-bold ${showArchivedCards ? 'bg-red-500 text-white' : 'text-gray-400'}`}>الأرشيف</button></div></div><div className="relative w-full sm:w-64"><Search className="absolute right-3 top-3 h-4 w-4 text-cyan-300" /><Input value={searchCards} onChange={e => setSearchCards(e.target.value)} placeholder="بحث باسم اللاعب..." className="pr-10 bg-[#1e2a4a] border-red-500/40 text-white" /></div></CardHeader>
                  <CardContent className="p-0"><div className="overflow-x-auto w-full custom-scrollbar"><table className="w-full text-right text-white"><thead className="bg-[#0a1428]"><tr><th className="p-4 text-cyan-300">اللاعب</th><th className="p-4 text-cyan-300">الفريق</th><th className="p-4 text-center text-cyan-300">البطاقات</th><th className="p-4 text-center text-cyan-300">الوضعية للمباراة القادمة</th></tr></thead>
                     <tbody>
                        {filteredCardsList.map((c, i) => (
                          <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                             <td className="p-4 font-bold text-white text-base">{c.player}</td><td className="p-4 text-sm text-gray-300">{c.team}</td>
                             <td className="p-4 text-center flex justify-center gap-4 font-black text-sm"><span className="bg-yellow-400/10 text-yellow-400 border border-yellow-400/30 px-2.5 py-0.5 rounded-md">🟨 {c.yellow}</span><span className="bg-red-500/10 text-red-500 border border-red-500/30 px-2.5 py-0.5 rounded-md">🟥 {c.red}</span></td>
                             <td className="p-4 text-center"><Badge className={c.status === "طرد" || c.status === "إيقاف" ? "bg-red-500/20 text-red-400 border border-red-500/50" : "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"}>{c.status === "طرد" ? "🔴 موقف للطرد" : c.status === "إيقاف" ? "❌ موقف للتراكم" : "✔️ متاح للمشاركة"}</Badge></td>
                          </tr>
                        ))}
                     </tbody></table></div>
                  </CardContent>
            ...
            {activeTab === "stats" && (
               <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     {[ { t: "إجمالي الأهداف", v: statsData.totalGoals, i: "⚽", c: "text-yellow-400" }, { t: "المباريات المكتملة", v: statsData.totalMatches, i: "⏱️", c: "text-cyan-400" }, { t: "الكروت الصفراء", v: statsData.totalYellow, i: "🟨", c: "text-yellow-500" }, { t: "الكروت الحمراء", v: statsData.totalRed, i: "🟥", c: "text-red-500" } ].map((card, idx) => (
                        <Card key={idx} className="bg-[#13213a] border border-white/5 p-4 flex flex-col items-center justify-center text-center shadow-md"><span className="text-3xl mb-1">{card.i}</span><span className="text-gray-400 text-xs font-bold">{card.t}</span><span className={`text-2xl sm:text-3xl font-black mt-2 ${card.c}`}>{card.v}</span></Card>
                     ))}
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <Card className="bg-[#13213a] border border-white/5 p-6 rounded-3xl space-y-4">
                        <h3 className="text-cyan-300 font-black text-lg border-b border-white/5 pb-2">🎯 القوة الهجومية والدفاعية بالأرقام</h3>
                        <div className="space-y-3">
                           <div className="flex justify-between items-center bg-[#0a1428] p-3 rounded-xl"><span className="text-gray-400 font-bold text-sm">أقوى خط هجوم (الأكثر تسجيلاً):</span><span className="text-emerald-400 font-black text-sm">{statsData.bestAttack.team} <Badge className="bg-emerald-500/20 text-emerald-400 font-black mr-2">{statsData.bestAttack.gf} أهداف</Badge></span></div>
                           <div className="flex justify-between items-center bg-[#0a1428] p-3 rounded-xl"><span className="text-gray-400 font-bold text-sm">أقوى خط دفاع (الأقل استقبالاً):</span><span className="text-emerald-400 font-black text-sm">{statsData.bestDefense.team} <Badge className="bg-emerald-500/20 text-emerald-400 font-black mr-2">{statsData.bestDefense.ga} أهداف</Badge></span></div>
                        </div>
                     </Card>
                     <Card className="bg-[#13213a] border border-white/5 p-6 rounded-3xl space-y-4">
                        <h3 className="text-cyan-300 font-black text-lg border-b border-white/5 pb-2">📈 المعدلات الرقمية لكل مباراة</h3>
                        <div className="space-y-3">
                           <div className="flex justify-between items-center bg-[#0a1428] p-3 rounded-xl"><span className="text-gray-400 font-bold text-sm">معدل التهديف العام:</span><span className="text-yellow-400 font-black text-sm">{statsData.goalsPerMatch} هدف / مباراة</span></div>
                           <div className="flex justify-between items-center bg-[#0a1428] p-3 rounded-xl"><span className="text-gray-400 font-bold text-sm">معدل الإنذارات الكلي:</span><span className="text-yellow-500 font-black text-sm">{statsData.yellowPerMatch} إنذار / مباراة</span></div>
                        </div>
                     </Card>
                  </div>
               </div>
            ...
            {activeTab === "motm_tab" && (
               <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] p-4 sm:p-6 animate-in fade-in duration-500">
                  <CardHeader className="pb-4 border-b border-white/10 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4"><div><CardTitle className="text-yellow-300 font-black">🌟 سجل لوحة شرف نجوم اللقاءات (Man Of The Match)</CardTitle></div><div className="relative w-full sm:w-64"><Search className="absolute right-3 top-3 h-4 w-4 text-cyan-300" /><Input value={searchMotm} onChange={e => setSearchMotm(e.target.value)} placeholder="بحث باسم النجم..." className="pr-10 bg-[#1e2a4a] border-yellow-400/40 text-white" /></div></CardHeader>
                  <CardContent className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                     {motmList.filter(m => !searchMotm || String(m.player).toLowerCase().includes(searchMotm.trim().toLowerCase())).map((m, idx) => (
                        <div key={idx} className="bg-gradient-to-b from-[#1e2a4a] to-[#0a1428] border border-yellow-400/30 rounded-2xl overflow-hidden p-4 flex gap-4 items-center shadow-lg relative group transition-transform hover:scale-[1.02]">
                           <div className="w-16 h-16 rounded-full bg-[#0a1428] border-2 border-yellow-400 overflow-hidden shrink-0 flex items-center justify-center">{m.imageUrl ? <img src={m.imageUrl} className="w-full h-full object-cover" alt="MOTM"/> : <span className="text-2xl">👤</span>}</div>
                           <div className="flex-1 min-w-0"><h4 className="font-black text-white text-base truncate">{m.player}</h4><span className="text-yellow-400 text-xs font-bold block mt-0.5 truncate">{m.team}</span><p className="text-[10px] text-gray-400 mt-2 truncate">مباراة: {m.matchName}</p></div>
                        </div>
                     ))}
                  </CardContent>
               </Card>
            ...
            {activeTab === "media" && (
               <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex justify-center mb-4"><div className="bg-[#1e2a4a] p-1 border border-white/5 rounded-xl flex gap-1"><button onClick={() => setMediaSubTab("news")} className={`text-sm px-4 py-2 rounded-lg font-bold ${mediaSubTab === 'news' ? 'bg-cyan-600 text-white' : 'text-gray-400'}`}>📰 الأخبار والمقالات</button><button onClick={() => setMediaSubTab("videos")} className={`text-sm px-4 py-2 rounded-lg font-bold ${mediaSubTab === 'videos' ? 'bg-cyan-600 text-white' : 'text-gray-400'}`}>🎥 استوديو الأهداف والملخصات</button></div></div>
                  {mediaSubTab === "news" ? (
                     <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {mediaItems.filter(item => item.type === "news").map(news => (
                           <Card key={news.id} className="bg-[#13213a] border border-white/5 rounded-3xl overflow-hidden shadow-md">
                              ...
                              <CardContent className="p-5 space-y-3"><h3 className="text-lg font-black text-white leading-snug">{news.title}</h3><p className="text-gray-300 text-sm font-bold leading-relaxed line-clamp-3">{news.body}</p></CardContent>
                           </Card>
                        ))}
                     </div>
                  ) : (
                        ...
                        {mediaItems.filter(item => item.type === "video").map(video => {
                           ...
                           return (
                             <Card key={video.id} className="bg-[#13213a] border border-white/5 rounded-3xl overflow-hidden p-4 shadow-xl space-y-4">
                                {ytId ? (
                                   <div className="w-full aspect-video rounded-2xl overflow-hidden bg-black border border-white/5 shadow-inner"><iframe src={`https://www.youtube.com/embed/${ytId}`} className="w-full h-full border-none" allowFullScreen title={video.title} /></div>
        ...
        {mainAppTab === 'elite_cup' && (
          <div className="space-y-6 animate-in fade-in duration-500">
             <div className="flex justify-center mb-8 px-2">
                <button onClick={() => { setActiveTab('rosters'); setMainAppTab('elite_cup'); setRosterViewMode('register'); setUnlockedRoster(null); setShowPaymentForm(true); }} className="bg-gradient-to-r from-indigo-600 to-blue-700 hover:from-indigo-500 hover:to-blue-600 text-white font-black py-4 px-10 rounded-2xl text-xl shadow-[0_0_25px_rgba(79,70,229,0.5)] transition-transform hover:scale-105 flex items-center gap-3">تسجيل فريق النخبة الآن ✍️ <Users/></button>
             </div>
             {rosterViewMode === 'list' && (
               <div className="text-center py-20 bg-[#13213a] border border-indigo-500/30 rounded-3xl shadow-xl max-w-4xl mx-auto">
                 <Trophy className="w-24 h-24 mx-auto text-indigo-400 mb-6 drop-shadow-lg" />
                 <h2 className="text-4xl font-black text-indigo-400 mb-4">بطولة كأس النخبة</h2>
                 <p className="text-white text-xl font-bold mb-8">استعدوا لأقوى المنافسات والأنظمة المتقدمة بمحافظة مطروح!</p>
               </div>
             )}
             {rosterViewMode === 'register' && !unlockedRoster && (
               <Card className="max-w-xl mx-auto bg-[#13213a] border border-indigo-500/30 rounded-3xl shadow-2xl mt-8">
                 <CardHeader className="text-center border-b border-white/5 pb-6"><Lock className="mx-auto h-12 w-12 text-indigo-400" /><CardTitle className="text-2xl font-black text-white mt-4">تسجيل فريق النخبة</CardTitle></CardHeader>
                 <CardContent className="p-8 space-y-6">
                    {showPaymentForm ? (
                       <div className="space-y-4 animate-in fade-in">
                       ...
                       <div className="space-y-4 animate-in fade-in">
                          <Input type="password" value={rosterAccessPassword} onChange={e => setRosterAccessPassword(e.target.value)} placeholder="أدخل الرقم السري المستلم للنخبة" className="bg-[#1e2a4a] border-indigo-500/50 text-white font-black text-center text-xl h-14" />
                          <Button onClick={handleRosterLogin} className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-black py-7 text-xl">الدخول لاستمارة النخبة <Unlock className="ml-2 h-5 w-5" /></Button>
                          <div className="text-center pt-2"><button onClick={() => setRosterViewMode('list')} className="text-gray-400 text-sm">إلغاء والعودة</button></div>
                       </div>
                    )}
                 </CardContent>
               </Card>
        ...
        {/* PLAYER REGISTRATION CARDS SECTION */}
        {mainAppTab === 'player_cards' && (
          <div className="space-y-8 animate-in fade-in duration-500 mt-4 print:bg-white player-cards-ui">
            <style>{`
            ...
            <div className="no-print bg-gradient-to-br from-[#45108A]/95 via-[#4D1689]/95 to-[#1f3d94]/95 border border-cyan-400/20 rounded-3xl p-6 shadow-2xl text-center md:text-right overflow-hidden relative">
              <div className="absolute -left-10 -top-10 w-40 h-40 rounded-full bg-green-400/20 blur-3xl"></div>
              <div className="absolute -right-10 -bottom-10 w-48 h-48 rounded-full bg-orange-400/20 blur-3xl"></div>
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row items-center gap-4 md:gap-5 justify-center md:justify-between">
                  <div>
                    <h2 className="text-3xl sm:text-4xl font-black text-white mb-2">🪪 بطاقة لاعب / مدير فني</h2>
                    <p className="text-white/80 font-bold leading-7">نسخة أفخم للكارت مع لوجو البطولة من الإدارة، وصورة داخل إطار احترافي، وتوقيع وختم واضحين، وظهر مخصص باسم البطولة ونوع البطاقة فقط.</p>
                  </div>
                  <div className="shrink-0 w-24 h-24 rounded-full bg-white/95 p-2 shadow-2xl">
                    <img src={PLAYER_CARD_BRAND_LOGO} alt="مطروح الرياضية" className="w-full h-full object-contain rounded-full" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="no-print flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
              {availablePlayerCardTournaments.map((t:any) => (
                <button key={t.id} onClick={() => setSelectedPlayerTournamentId(t.id)} className={`shrink-0 px-5 py-3 rounded-2xl font-black border transition-all ${selectedPlayerTournamentId === t.id ? 'bg-gradient-to-r from-[#46d834] via-[#1da1f2] to-[#f59e0b] text-white border-white/30 shadow-lg scale-105' : 'bg-[#13213a] text-white border-cyan-400/20 hover:border-cyan-400/60'}`}>
                  {t.name}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[430px_1fr] gap-6 items-start">
              <Card className="no-print bg-[#13213a] border border-cyan-400/30 rounded-3xl shadow-2xl overflow-hidden">
                <CardHeader className="bg-[#0a1428]/70 border-b border-white/10">
                  <CardTitle className="text-2xl font-black text-cyan-300">بيانات الكارت</CardTitle>
                </CardHeader>
                <CardContent className="p-6 space-y-4">
                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">اسم البطولة</label>
                    <select value={selectedPlayerTournamentId} onChange={e => setSelectedPlayerTournamentId(e.target.value)} className="w-full h-12 rounded-xl bg-[#0a1428] border border-cyan-400/20 text-white font-black px-3 outline-none focus:border-cyan-400">
                      {availablePlayerCardTournaments.map((t:any) => <option key={t.id} value={t.id}>{t.name}</option>)}
                    </select>
                  </div>

                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">الاسم</label>
                    <Input placeholder="اكتب الاسم" value={playerCardForm.fullName} onChange={e => setPlayerCardForm((p:any) => ({ ...p, fullName: e.target.value }))} className="bg-[#0a1428] border-cyan-400/20 text-white font-bold h-12" />
                  </div>

                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">الصفة</label>
                    <select value={playerCardForm.role} onChange={e => setPlayerCardForm((p:any) => ({ ...p, role: e.target.value }))} className="w-full h-12 rounded-xl bg-[#0a1428] border border-cyan-400/20 text-white font-black px-3 outline-none focus:border-cyan-400">
                      {PLAYER_CARD_ROLE_OPTIONS.map((r:any) => <option key={r.value} value={r.value}>{r.label}</option>)}
                    </select>
                  </div>

                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">اسم الفريق</label>
                    <Input placeholder="اتركه فارغًا لو لاعب حر" value={playerCardForm.teamName} onChange={e => setPlayerCardForm((p:any) => ({ ...p, teamName: e.target.value }))} className="bg-[#0a1428] border-cyan-400/20 text-white font-bold h-12" />
                  </div>

                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">تاريخ الميلاد</label>
                    <Input type="date" value={playerCardForm.birthDate} onChange={e => setPlayerCardForm((p:any) => ({ ...p, birthDate: e.target.value }))} className="bg-[#0a1428] border-cyan-400/20 text-white font-bold h-12" />
                  </div>

                  <div>
                    <label className="block text-sm text-gray-300 font-black mb-2">الرقم القومي</label>
                    <Input inputMode="numeric" placeholder="الرقم القومي" value={playerCardForm.nationalId} onChange={e => setPlayerCardForm((p:any) => ({ ...p, nationalId: e.target.value }))} className="bg-[#0a1428] border-cyan-400/20 text-white font-bold h-12" dir="ltr" />
                  </div>

                  <div className="rounded-2xl border border-cyan-400/20 bg-[#0a1428]/70 p-4 space-y-4">
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      <div>
                        <div className="text-white font-black">صورة المشارك</div>
                        <div className="text-xs text-gray-400 font-bold">ارفع الصورة ثم عدّل موضعها داخل الإطار</div>
                      </div>
                      <label className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan-400 text-black font-black cursor-pointer hover:bg-cyan-300 transition-colors">
                        <Upload className="w-4 h-4" /> رفع صورة
                        <input type="file" accept="image/*" className="hidden" onChange={e => handlePlayerCardPhotoUpload(e.target.files?.[0])} />
                      </label>
                    </div>

                    <div className="mx-auto w-[170px] h-[220px] rounded-2xl overflow-hidden border-2 border-dashed border-cyan-300/40 bg-slate-900 relative">
                      {(playerCardForm.photoPreview || playerCardForm.photoUrl) ? (
                        <img
                          src={playerCardForm.photoPreview || playerCardForm.photoUrl}
                          alt="معاينة الصورة"
                          className="absolute inset-0 w-full h-full object-cover"
                          style={{ objectPosition: `${Number(playerCardForm.cropX || 50)}% ${Number(playerCardForm.cropY || 50)}%`, transform: `scale(${Number(playerCardForm.zoom || 1)})` }}
                        />
                      ) : (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-400 p-4">
                          <Camera className="w-8 h-8 mb-2" />
                          <span className="font-black text-sm">مكان الصورة</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center justify-between text-xs text-gray-300 font-bold mb-1"><span>تكبير / تصغير</span><span>{Number(playerCardForm.zoom || 1).toFixed(2)}x</span></div>
                        <input type="range" min="1" max="2.5" step="0.05" value={playerCardForm.zoom || 1} onChange={e => setPlayerCardForm((p:any) => ({ ...p, zoom: Number(e.target.value) }))} className="w-full accent-cyan-400" />
                      </div>
                      <div>
                        <div className="flex items-center justify-between text-xs text-gray-300 font-bold mb-1"><span>تحريك أفقي</span><span>{Number(playerCardForm.cropX || 50)}%</span></div>
                        <input type="range" min="0" max="100" step="1" value={playerCardForm.cropX || 50} onChange={e => setPlayerCardForm((p:any) => ({ ...p, cropX: Number(e.target.value) }))} className="w-full accent-green-400" />
                      </div>
                      <div>
                        <div className="flex items-center justify-between text-xs text-gray-300 font-bold mb-1"><span>تحريك رأسي</span><span>{Number(playerCardForm.cropY || 50)}%</span></div>
                        <input type="range" min="0" max="100" step="1" value={playerCardForm.cropY || 50} onChange={e => setPlayerCardForm((p:any) => ({ ...p, cropY: Number(e.target.value) }))} className="w-full accent-orange-400" />
                      </div>
                    </div>
                  </div>
              ...
              <div id="player-card-print-zone" className="space-y-4">
                <div className="print-card-grid grid grid-cols-1 md:grid-cols-2 gap-5 justify-items-center">
                  <div className="id-card-print relative w-full max-w-[430px] aspect-[1.586/1] rounded-[22px] overflow-hidden shadow-2xl border border-[#d5d5d5] text-black bg-white">
                    ...
                    <div className="relative z-10 p-4 h-full flex flex-col" style={{ fontFamily: PLAYER_CARD_FONT_FAMILY }}>
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-12 h-12 rounded-full bg-white border border-gray-200 shadow-sm p-1.5 overflow-hidden">
                            <img src={PLAYER_CARD_BRAND_LOGO} className="w-full h-full object-contain rounded-full" alt="مطروح الرياضية" />
                          </div>
                          <div>
                            <div className="text-[#4b1690] text-[16px] leading-none font-black">مطروح الرياضية</div>
                            <div className="text-[9px] text-gray-500 font-bold mt-1">بطاقة تعريف معتمدة</div>
                          </div>
                        </div>
                        <div className="text-left flex flex-col items-center gap-1">
                          <div className="w-12 h-12 object-contain bg-transparent border-0 p-0 overflow-visible">{currentPlayerCardPreview.tournamentLogoUrl ? <img src={currentPlayerCardPreview.tournamentLogoUrl} className="w-full h-full object-contain drop-shadow-[0_6px_10px_rgba(0,0,0,0.25)]" alt="بطولة" /> : <img src={PLAYER_CARD_BRAND_LOGO} className="w-full h-full object-contain drop-shadow-[0_6px_10px_rgba(0,0,0,0.25)]" alt="بطولة" />}</div>
                          <div className="text-[8px] text-gray-500 font-black max-w-[105px] leading-tight text-center">{currentPlayerCardPreview.tournamentName}</div>
                        </div>
                      </div>

                      <div className="flex gap-3 flex-1 min-h-0 mt-1">
                        <div className="w-[103px] shrink-0 flex flex-col items-center">
                          <div className="relative w-[94px] h-[121px] rounded-[18px] overflow-hidden border-2 border-[#4b1690]/20 bg-gradient-to-b from-slate-100 to-slate-200 shadow-inner">
                            {currentPlayerCardPreview.photoUrl ? (
                              <img
                                src={currentPlayerCardPreview.photoUrl}
                                alt="صورة المشارك"
                                className="absolute inset-0 w-full h-full object-cover"
                                style={{ objectPosition: `${Number(currentPlayerCardPreview.cropX || 50)}% ${Number(currentPlayerCardPreview.cropY || 50)}%`, transform: `scale(${Number(currentPlayerCardPreview.zoom || 1)})` }}
                              />
                            ) : (
                          ...
                          <div className="mt-2 text-[9px] text-white font-black px-3 py-1 rounded-full bg-gradient-to-r from-[#4b1690] to-[#1da1f2] shadow-md">{currentPlayerCardPreview.role === 'manager' ? 'مدير فني' : 'لاعب'}</div>
                          <div className="mt-1 text-[7px] text-slate-600 font-black" dir="ltr">{currentPlayerCardPreview.serialNumber}</div>
                        </div>

                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                          <div className="space-y-1.5 text-right">
                            <div>
                              <div className="text-[9px] text-gray-500 font-black">الاسم</div>
                              <div className="text-[16px] leading-tight font-black text-[#111827] border-b border-[#e6e6e6] pb-0.5 min-h-[24px]">{currentPlayerCardPreview.fullName || '................'}</div>
                            </div>
                            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]">
                              <div>
                                <div className="text-[8px] text-gray-500 font-black">الصفة</div>
                                <div className="font-black text-[#4b1690]">{currentPlayerCardPreview.roleLabel}</div>
                              </div>
                              <div>
                                <div className="text-[8px] text-gray-500 font-black">الفريق</div>
                                <div className="font-black text-[#0f766e] truncate">{currentPlayerCardPreview.teamName || 'لاعب حر'}</div>
                              </div>
                              <div>
                                <div className="text-[8px] text-gray-500 font-black">تاريخ الميلاد</div>
                                <div className="font-black">{currentPlayerCardPreview.birthDate || '----/--/--'}</div>
                              </div>
                              <div>
                                <div className="text-[8px] text-gray-500 font-black">تاريخ التسجيل</div>
                                <div className="font-black">{currentPlayerCardPreview.registrationDate}</div>
                              </div>
                              <div className="col-span-2">
                                <div className="text-[8px] text-gray-500 font-black">الرقم القومي</div>
                                <div className="font-black tracking-wide" dir="ltr">{currentPlayerCardPreview.nationalId || '00000000000000'}</div>
                              </div>
                            </div>
                          </div>

                          <div className="pt-2 grid grid-cols-[70px_1fr_54px] items-end gap-2 border-t border-[#ececec] mt-2">
                            <div className="relative w-[62px] h-[62px] rounded-full border-[2.4px] border-red-600 text-red-700 overflow-hidden flex items-center justify-center bg-red-50/70 shadow-[0_0_0_2px_rgba(220,38,38,0.16)]">
                              <div className="absolute inset-[5px] rounded-full border border-red-500/60"></div>
                              <img src={PLAYER_CARD_BRAND_LOGO} alt="ختم" className="absolute inset-0 w-full h-full object-contain opacity-12" />
                              <div className="relative z-10 text-center leading-tight -rotate-6">
                                <div className="text-[9px] font-black">مطروح</div>
                                <div className="text-[10px] font-black">معتمد</div>
                                <div className="text-[7px] font-black">OFFICIAL</div>
                              </div>
                            </div>
                            ...
                            <div className="text-center min-w-0">
                              <div className="flex items-end justify-center gap-2">
                                <div className="min-w-[116px]">
                                  <div className="text-[10px] font-black text-gray-600 mb-1">يعتمد</div>
                                  <div className="h-[34px] flex items-end justify-center bg-white rounded-md border border-dashed border-slate-300 px-1">
                                    <img src={PLAYER_CARD_SIGNATURE_IMAGE} alt="التوقيع" className="max-h-[32px] w-auto object-contain opacity-100 drop-shadow-sm" />
                                    <span className="sr-only">فتحي هيرو</span>
                                  </div>
                                </div>
                                <div className="flex-1 min-w-[80px]">
                                  <div className="h-[18px] rounded-sm overflow-hidden bg-[repeating-linear-gradient(90deg,#111_0_1px,transparent_1px_3px,#111_3px_5px,transparent_5px_8px,#111_8px_9px,transparent_9px_12px)]"></div>
                                  <div className="text-[7px] text-slate-700 font-black mt-0.5" dir="ltr">{currentPlayerCardPreview.serialNumber}</div>
                                </div>
                              </div>
                            </div>

                            <div className="w-[48px] h-[48px] rounded-md bg-white border border-slate-200 p-0.5 overflow-hidden">
                  ...
                  <div className="id-card-print relative w-full max-w-[430px] aspect-[1.586/1] rounded-[22px] overflow-hidden shadow-2xl border border-[#4b1690]/20 text-white">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-[0.14]">
                      <img src={PLAYER_CARD_BRAND_LOGO} className="w-[65%] h-[65%] object-contain" alt="Watermark" />
                    </div>
                    <div className="relative z-10 h-full flex flex-col items-center justify-center text-center p-6" style={{ fontFamily: PLAYER_CARD_FONT_FAMILY }}>
                      <div className="w-20 h-20 rounded-full bg-white/12 backdrop-blur-sm p-2 shadow-2xl border border-white/20 overflow-hidden">
                        <img src={PLAYER_CARD_BRAND_LOGO} className="w-full h-full object-contain" alt="مطروح الرياضية" />
                      </div>
                      <div className="mt-5 text-[24px] leading-tight font-black text-white max-w-[280px]">{currentPlayerCardPreview.tournamentName}</div>
                      <div className="mt-3 text-[15px] text-yellow-300 font-black tracking-wide">{currentPlayerCardPreview.role === 'manager' ? 'بطاقة مدير فني' : 'بطاقة لاعب'}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-16 border-t border-white/5 pt-6 pb-2 flex flex-col items-center justify-center text-center">
           <div className="text-gray-400 text-sm font-bold flex items-center gap-2"><span>إدارة المنصة</span><Badge className="bg-[#13213a] text-yellow-400 border border-yellow-400/20 px-3 py-1 font-black text-sm cursor-default shadow-md">فتحي هيرو 🦅</Badge></div>
           <div className="text-cyan-300 text-[10px] mt-2 opacity-60 font-bold tracking-wider">جميع الحقوق محفوظة © 2026 لمنصة مطروح الرياضية</div>
        </div>
