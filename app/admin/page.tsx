...
export default function AdminPage() {
  ...
  const addOrUpdateGoal = async () => {
    ...
    const data = { player: goalForm.player.trim(), team: goalForm.team, goals: goalForm.goalsCount };
    if (editingGoalId) { await updateDoc(doc(db, "goals", editingGoalId), data); setEditingGoalId(null); alert("✅ تم تعديل الهدف");} 
    else { await addDoc(collection(db, "goals"), data); alert("✅ تم إضافة الهدف");}
    setGoalForm({ player: "", team: TEAM_NAMES[0], goalsCount: 1 });
  };
  const startEditGoal = (goal: GoalEvent) => { setEditingGoalId(goal.id); setGoalForm({ player: goal.player, team: goal.team, goalsCount: goal.goals }); };
  ...
  // حماية الفلاتر من الأخطاء (حل المشكلة)
  const safeGoalSearch = goalSearchTerm.toLowerCase();
  const filteredGoals = goals.filter(g => 
    String(g.player || "").toLowerCase().includes(safeGoalSearch) || 
    String(g.team || "").toLowerCase().includes(safeGoalSearch)
  );

  const safeCardSearch = cardSearchTerm.toLowerCase();
  const filteredCards = cardEvents.filter(c => 
    String(c.player || "").toLowerCase().includes(safeCardSearch) || 
    String(c.team || "").toLowerCase().includes(safeCardSearch)
  );
    ...
    <div dir="rtl" className="min-h-screen bg-[#0a1428] text-white">
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)}>
              ...
              <Card key={match.id} className="bg-[#1e2a4a] border-2 border-red-500/80 overflow-hidden shadow-[0_0_30px_rgba(239,68,68,0.2)]">
                ...
                <div className="bg-red-600 text-white px-6 py-3 flex flex-wrap justify-between items-center gap-4">
                  <div className="flex items-center gap-4">
                     ...
                     <select 
                       value={match.status || "الشوط الأول"} 
                       onChange={(e) => updateMatchLive(match.id, { status: e.target.value })}
                       className="bg-black/40 border-none text-white font-bold rounded-lg px-3 py-1 text-sm outline-none cursor-pointer"
                     >
                        <option value="الشوط الأول">الشوط الأول</option>
                        <option value="استراحة">استراحة</option>
                        <option value="الشوط الثاني">الشوط الثاني</option>
                        <option value="وقت إضافي">وقت إضافي</option>
                        <option value="ضربات جزاء">ضربات جزاء</option>
                        <option value="انتهت">انتهت</option>
                     </select>
                  </div>
                  <Button size="sm" variant="outline" className="bg-white/10 text-white border-white/20 hover:bg-white/20 hover:text-white" onClick={() => updateMatchLive(match.id, { isLive: false })}>إغلاق البث (نقل للسابق)</Button>
                </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex justify-center items-center gap-6 mb-8 bg-[#13213a] p-4 rounded-2xl border border-white/5 flex-wrap">
                     <Button 
                       onClick={() => updateMatchLive(match.id, { isTimerRunning: !match.isTimerRunning })}
                       className={`font-bold h-12 px-6 ${match.isTimerRunning ? 'bg-amber-500 hover:bg-amber-600' : 'bg-emerald-500 hover:bg-emerald-600'}`}
                     >
                       {match.isTimerRunning ? <><Pause className="mr-2 h-5 w-5" /> إيقاف التايمر</> : <><Play className="mr-2 h-5 w-5" /> تشغيل التايمر</>}
                     </Button>
                     <div className="flex items-center gap-3">
                        <label className="text-gray-400 font-bold">الدقيقة:</label>
                        <Input type="number" value={match.liveMinute || 0} onChange={(e) => updateMatchLive(match.id, { liveMinute: Number(e.target.value) })} className="w-24 text-center text-2xl font-black bg-black border-yellow-400 text-yellow-400" />
                     </div>
                  </div>

                  {/* الفرق والأهداف والكروت */}
                  <div className="grid md:grid-cols-3 gap-8 items-center mb-8">
                    ...
                    <div className="text-center bg-[#13213a] p-6 rounded-3xl border border-white/5 relative">
                      ...
                      <div className="flex items-center justify-center gap-4 mb-6">
                        <Button variant="outline" className="h-12 w-12 rounded-full border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black" onClick={() => updateMatchLive(match.id, { homeGoals: (match.homeGoals || 0) + 1 })}><Plus /></Button>
                        <span className="text-7xl font-black text-white w-20">{match.homeGoals || 0}</span>
                        <Button variant="outline" className="h-12 w-12 rounded-full border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black" onClick={() => updateMatchLive(match.id, { homeGoals: Math.max(0, (match.homeGoals || 0) - 1) })}><Minus /></Button>
                      </div>
                      <div className="flex justify-center items-center gap-2 bg-red-500/10 py-2 rounded-xl border border-red-500/20">
                         ...
                         <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-white" onClick={() => updateMatchLive(match.id, { redCardsHome: Math.max(0, (match.redCardsHome || 0) - 1) })}><Minus className="h-4 w-4" /></Button>
                         <span className="font-bold text-xl text-red-500">{match.redCardsHome || 0}</span>
                         <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-white" onClick={() => updateMatchLive(match.id, { redCardsHome: (match.redCardsHome || 0) + 1 })}><Plus className="h-4 w-4" /></Button>
                      </div>
                    </div>
                    ...
                    <div className="text-center bg-[#13213a] p-6 rounded-3xl border border-white/5 relative">
                      ...
                      <div className="flex items-center justify-center gap-4 mb-6">
                        <Button variant="outline" className="h-12 w-12 rounded-full border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black" onClick={() => updateMatchLive(match.id, { awayGoals: (match.awayGoals || 0) + 1 })}><Plus /></Button>
                        <span className="text-7xl font-black text-white w-20">{match.awayGoals || 0}</span>
                        <Button variant="outline" className="h-12 w-12 rounded-full border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black" onClick={() => updateMatchLive(match.id, { awayGoals: Math.max(0, (match.awayGoals || 0) - 1) })}><Minus /></Button>
                      </div>
                      <div className="flex justify-center items-center gap-2 bg-red-500/10 py-2 rounded-xl border border-red-500/20">
                         <span className="text-red-500 font-bold">طرد:</span>
                         <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-white" onClick={() => updateMatchLive(match.id, { redCardsAway: Math.max(0, (match.redCardsAway || 0) - 1) })}><Minus className="h-4 w-4" /></Button>
                         <span className="font-bold text-xl text-red-500">{match.redCardsAway || 0}</span>
                         <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-white" onClick={() => updateMatchLive(match.id, { redCardsAway: (match.redCardsAway || 0) + 1 })}><Plus className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  </div>
                  ...
                  {match.status === "ضربات جزاء" && (
                    <div className="bg-[#13213a] border border-yellow-400/30 p-6 rounded-3xl mt-6">
                      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        ...
                        <Button size="sm" variant="outline" onClick={() => addPenaltySlot(match.id)} className="border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-black">
                           <Plus className="ml-2 h-4 w-4" /> إضافة ركلة إضافية
                        </Button>
                      </div>
                      <div className="flex flex-col md:flex-row justify-between items-center bg-[#0a1428] p-4 rounded-xl gap-6">
                        ...
                        <div className="flex gap-2 flex-wrap justify-center">
                           {(match.penaltiesHome || ['none','none','none','none','none']).map((p, i) => (
                             <button key={i} onClick={() => togglePenalty(match.id, 'home', i, p)} className={`w-10 h-10 rounded-full border-2 transition-all flex items-center justify-center ${p === 'scored' ? 'bg-emerald-500 border-emerald-400' : p === 'missed' ? 'bg-red-500 border-red-400' : 'bg-[#1e2a4a] border-gray-600'}`}>
                               {p === 'scored' && <span className="text-white font-bold">✔</span>}
                               {p === 'missed' && <span className="text-white font-bold">✖</span>}
                             </button>
                           ))}
                        </div>
                        <div className="text-3xl font-black text-yellow-400 bg-[#1e2a4a] px-6 py-2 rounded-xl">
                           {(match.penaltiesHome || []).filter(p => p === 'scored').length} - {(match.penaltiesAway || []).filter(p => p === 'scored').length}
                        </div>
                        {/* ركلات الفريق الثاني */}
                        <div className="flex gap-2 flex-wrap justify-center">
                           {(match.penaltiesAway || ['none','none','none','none','none']).map((p, i) => (
                             <button key={i} onClick={() => togglePenalty(match.id, 'away', i, p)} className={`w-10 h-10 rounded-full border-2 transition-all flex items-center justify-center ${p === 'scored' ? 'bg-emerald-500 border-emerald-400' : p === 'missed' ? 'bg-red-500 border-red-400' : 'bg-[#1e2a4a] border-gray-600'}`}>
                               {p === 'scored' && <span className="text-white font-bold">✔</span>}
                               {p === 'missed' && <span className="text-white font-bold">✖</span>}
                             </button>
                           ))}
          ...
          {/* إضافة مباراة جديدة */}
          <Card className="border-yellow-400 bg-[#13213a] mb-8 mt-8">
            ...
            <CardContent className="space-y-6 p-6 text-white">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <select value={matchForm.teamA} onChange={e => setMatchForm(p => ({...p, teamA: e.target.value}))} className="bg-[#1e2a4a] border border-yellow-400 rounded-2xl p-4 text-white">{sortedTeams.map(t => <option key={t} value={t}>{t}</option>)}</select>
                <div className="flex items-center justify-center text-5xl text-yellow-400 font-black">VS</div>
                <select value={matchForm.teamB} onChange={e => setMatchForm(p => ({...p, teamB: e.target.value}))} className="bg-[#1e2a4a] border border-yellow-400 rounded-2xl p-4 text-white">{sortedTeams.map(t => <option key={t} value={t}>{t}</option>)}</select>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div><label className="block mb-2 text-cyan-300">أهداف {matchForm.teamA}</label><Input type="number" value={matchForm.homeGoals} onChange={e => setMatchForm(p => ({...p, homeGoals: Number(e.target.value)}))} className="text-6xl text-center h-20 text-white" /></div>
                <div><label className="block mb-2 text-cyan-300">أهداف {matchForm.teamB}</label><Input type="number" value={matchForm.awayGoals} onChange={e => setMatchForm(p => ({...p, awayGoals: Number(e.target.value)}))} className="text-6xl text-center h-20 text-white" /></div>
              </div>
              ...
              <Button onClick={saveMatch} className="w-full bg-yellow-400 text-black font-bold py-7 text-xl">{editingId ? "حفظ التعديل" : "إضافة المباراة"}</Button>
            </CardContent>
          </Card>
          ...
          <TabsContent value="all">
            <Card className="border-yellow-400 bg-[#13213a]">
              <CardHeader className="flex flex-col sm:flex-row justify-between gap-4"><CardTitle className="text-yellow-300">المباريات السابقة</CardTitle><Input placeholder="ابحث عن فريق..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-sm border-yellow-400 bg-[#1e2a4a] text-white" /></CardHeader>
              <CardContent className="space-y-3">
                {matches.filter(m => !m.isLive && (String(m.teamA || "").includes(searchTerm) || String(m.teamB || "").includes(searchTerm))).map(m => (
                  <div key={m.id} className="bg-[#1e2a4a] p-4 rounded-2xl flex flex-col sm:flex-row justify-between gap-4 border border-yellow-400/30">
                    <div><div className="font-bold text-white">{m.teamA} {m.homeGoals} - {m.awayGoals} {m.teamB}</div><div className="text-cyan-300 text-sm">{m.date} • {m.status || m.time}</div></div>
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" onClick={() => { updateMatchLive(m.id, { isLive: true, status: m.status || "الشوط الأول", liveMinute: m.liveMinute || 1 }); setActiveTab("live"); }} className="bg-red-600 hover:bg-red-700 text-white"><Play className="ml-1 h-4 w-4" /> مباشر</Button>
          ...
          <TabsContent value="today">
             <Card className="border-yellow-400 bg-[#13213a]">
              <CardHeader><CardTitle className="text-yellow-300">مباريات اليوم (بدون نتيجة)</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {matches.filter(m => m.date === todayStr && !m.isLive && m.homeGoals === 0 && m.awayGoals === 0).map(m => (
                  <div key={m.id} className="bg-[#1e2a4a] p-4 rounded-2xl flex flex-col sm:flex-row justify-between items-center border-l-4 border-yellow-400 gap-4">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => { updateMatchLive(m.id, { isLive: true, status: "الشوط الأول", liveMinute: 1 }); setActiveTab("live"); }} className="bg-red-600 text-white hover:bg-red-700"><Play className="ml-1 h-4 w-4" /> ابدأ البث</Button>
            ...
            <Card className="border-yellow-400 bg-[#13213a]">
              ...
              <CardContent className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-[#1e2a4a] rounded-2xl">
                  ...
                  <Button onClick={addOrUpdateGoal} className="bg-yellow-400 text-black font-bold">{editingGoalId ? "تعديل" : "إضافة"}</Button>
                </div>
                <div className="space-y-3">
                  {filteredGoals.map(goal => (
                    <Card key={goal.id} className="bg-[#1e2a4a] border border-yellow-400/30"><CardContent className="p-4 flex justify-between items-center text-white"><div><div className="font-bold">{goal.player}</div><div className="text-cyan-300">{goal.team} — {goal.goals} هدف</div></div><div className="flex gap-2"><Button size="sm" onClick={() => startEditGoal(goal)} className="bg-yellow-400 text-black"><Edit className="ml-2 h-4 w-4" /> تعديل</Button><Button size="sm" variant="destructive" onClick={() => deleteGoal(goal.id)}><Trash2 className="ml-2 h-4 w-4" /> حذف</Button></div></CardContent></Card>
                  ))}
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="cards">
            <Card className="border-yellow-400 bg-[#13213a]">
              <CardHeader><CardTitle className="text-yellow-300">إدارة الإنذارات والبطاقات</CardTitle><Input placeholder="ابحث عن لاعب..." value={cardSearchTerm} onChange={(e) => setCardSearchTerm(e.target.value)} className="mt-4 max-w-md border-yellow-400 bg-[#1e2a4a] text-white" /></CardHeader>
              <CardContent className="space-y-6 p-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-[#1e2a4a] rounded-2xl">
                  ...
                  <select value={cardForm.team} onChange={e => setCardForm(p => ({...p, team: e.target.value}))} className="bg-[#0a1428] border border-yellow-400 rounded-2xl p-3 text-white">{sortedTeams.map(t => <option key={t} value={t}>{t}</option>)}</select>
                  ...
                  {filteredCards.map(item => (
                    <Card key={item.id} className="bg-[#1e2a4a] border border-yellow-400/30">
                      <CardContent className="p-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-white">
                        <div><div className="font-bold text-lg">{item.player}</div><div className="text-cyan-300">{item.team}</div></div>
                        <div className="flex items-center gap-6">
                          <div className="flex items-center gap-2"><Button size="sm" variant="outline" onClick={() => updateCard(item.id, Math.max(0, item.yellow - 1), item.red)} className="border-yellow-400 text-white"><Minus className="h-4 w-4" /></Button><span className="px-4 py-1 bg-yellow-400/20 text-yellow-300 font-bold rounded">🟨 {item.yellow}</span><Button size="sm" variant="outline" onClick={() => updateCard(item.id, item.yellow + 1, item.red)} className="border-yellow-400 text-white"><Plus className="h-4 w-4" /></Button></div>
                          <div className="flex items-center gap-2"><Button size="sm" variant="outline" onClick={() => updateCard(item.id, item.yellow, Math.max(0, item.red - 1))} className="border-red-400 text-white"><Minus className="h-4 w-4" /></Button><span className="px-4 py-1 bg-red-500/20 text-red-300 font-bold rounded">🟥 {item.red}</span><Button size="sm" variant="outline" onClick={() => updateCard(item.id, item.yellow, item.red + 1)} className="border-red-400 text-white"><Plus className="h-4 w-4" /></Button></div>
