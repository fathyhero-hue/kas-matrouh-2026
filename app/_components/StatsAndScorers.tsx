...
interface StatsAndScorersProps {
  activeTab: string;
...
export const StatsAndScorers = ({
  ...
  return (
    <div className="animate-in fade-in duration-500">
        ...
        <Card className="rounded-3xl border border-yellow-400/30 bg-[#13213a] p-4 sm:p-6">
          <CardHeader className="flex flex-col sm:flex-row justify-between items-center gap-4 pb-4 border-b border-white/10 mb-4">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <Input value={searchScorers} onChange={e => setSearchScorers(e.target.value)} placeholder="بحث عن لاعب..." className="pr-10 bg-[#1e2a4a] border-yellow-400/50 text-white rounded-xl" />
            </div>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredScorers.map((s, i) => (
              <div key={i} className="bg-[#1e2a4a] p-4 rounded-2xl border border-white/5 flex items-center justify-between shadow-md">
                ...
                <Badge className="bg-cyan-500/20 text-cyan-400 text-lg font-black border border-cyan-500/30 px-3.5 py-1" dir="ltr">{s.goals} ⚽</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      {/* 2. تبويب الكروت والعقوبات */}
      {activeTab === "cards" && (
        <Card className="rounded-3xl border border-red-500/30 bg-[#13213a] p-4 sm:p-6">
          <CardHeader className="flex flex-col sm:flex-row justify-between items-center gap-4 pb-4 border-b border-white/10 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-1 border border-white/10 rounded-xl p-1 bg-[#0a1428]">
                <button onClick={() => setShowArchivedCards(false)} className={`text-xs px-3 py-1 rounded-lg font-bold ${!showArchivedCards ? 'bg-red-500 text-white' : 'text-gray-400'}`}>الحالية</button>
                <button onClick={() => setShowArchivedCards(true)} className={`text-xs px-3 py-1 rounded-lg font-bold ${showArchivedCards ? 'bg-red-500 text-white' : 'text-gray-400'}`}>الأرشيف</button>
              </div>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="absolute right-3 top-3 h-4 w-4 text-cyan-300" />
              <Input value={searchCards} onChange={e => setSearchCards(e.target.value)} placeholder="بحث باسم اللاعب..." className="pr-10 bg-[#1e2a4a] border-red-500/40 text-white" />
            </div>
          </CardHeader>
      ...
      {/* 3. تبويب الإحصائيات الرقمية */}
      {activeTab === "stats" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { t: "إجمالي الأهداف", v: statsData.totalGoals, i: "⚽", c: "text-yellow-400" },
              { t: "المباريات المكتملة", v: statsData.totalMatches, i: "⏱️", c: "text-cyan-400" },
              { t: "الكروت الصفراء", v: statsData.totalYellow, i: "🟨", c: "text-yellow-500" },
              { t: "الكروت الحمراء", v: statsData.totalRed, i: "🟥", c: "text-red-500" }
            ].map((card, idx) => (
              <Card key={idx} className="bg-[#13213a] border border-white/5 p-4 flex flex-col items-center justify-center text-center shadow-md">
                <span className="text-3xl mb-1">{card.i}</span>
                <span className="text-gray-400 text-xs font-bold">{card.t}</span>
                <span className={`text-2xl sm:text-3xl font-black mt-2 ${card.c}`}>{card.v}</span>
              </Card>
            ))}
          </div>
        </div>
