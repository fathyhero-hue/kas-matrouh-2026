...
import { Trophy, LogOut, Edit, Trash2, Plus, Minus, Play, Square, Pause, BellRing } from "lucide-react";
...
type AdminMatch = {
  id: string; teamA: string; teamB: string; homeGoals: number; awayGoals: number;
  round: string; date: string; time: string; dayName: string; status?: string;
  liveMinute?: number; isLive?: boolean; isTimerRunning?: boolean;
  redCardsHome?: number; redCardsAway?: number;
  penaltiesHome?: ("scored" | "missed" | "none")[]; penaltiesAway?: ("scored" | "missed" | "none")[];
};
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"live" | "all" | "today" | "tomorrow" | "goals" | "cards" | "ticker" | "notify">("live");
  ...
  useEffect(() => {
    ...
    const unsubMatches = onSnapshot(collection(db, "matches"), (snap) => {
      setMatches(snap.docs.map(d => {
        const data = d.data();
        return { id: d.id, ...data, teamA: cleanTeamString(data.teamA), teamB: cleanTeamString(data.teamB) } as AdminMatch;
      }));
    });
    const unsubGoals = onSnapshot(collection(db, "goals"), (snap) => setGoals(snap.docs.map(d => ({ id: d.id, ...d.data(), team: cleanTeamString(d.data().team) } as GoalEvent))));
    const unsubCards = onSnapshot(collection(db, "cards"), (snap) => setCardEvents(snap.docs.map(d => ({ id: d.id, ...d.data(), team: cleanTeamString(d.data().team) } as CardEvent))));
  ...
  const startEdit = (match: AdminMatch) => { setEditingId(match.id); setMatchForm({ teamA: match.teamA, teamB: match.teamB, homeGoals: match.homeGoals, awayGoals: match.awayGoals, round: match.round, date: match.date, time: match.time }); };
  ...
  const startEditGoal = (goal: GoalEvent) => { setEditingGoalId(goal.id); setGoalForm({ player: goal.player, team: goal.team, goalsCount: goal.goals, imageUrl: goal.imageUrl || "" }); };
  ...
  const addCard = async () => {
    ...
    setCardForm(p => ({ ...p, player: "" })); alert("✅ تم إضافة البطاقة");
  };
  ...
  // 🔔 دالة إرسال الإشعار للجمهور
  const sendNotification = async () => {
    if (!notifyTitle || !notifyBody) return alert("اكتب عنوان وتفاصيل الإشعار أولاً يا بطل!");
    setIsSending(true);
    
    try {
      // هنجيب كل أكواد المشتركين من الفايربيس
      const snap = await getDocs(collection(db, "subscribers"));
      const tokens = snap.docs.map(doc => doc.data().token);

      if (tokens.length === 0) {
        alert("مفيش حد اشترك في الإشعارات لسه!");
        setIsSending(false);
        return;
      }

      // إرسال البيانات للـ API اللي هنعمله الخطوة الجاية
      const res = await fetch("/api/notify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: notifyTitle, body: notifyBody, tokens })
      });

      if (res.ok) {
        alert(`✅ تم إرسال الإشعار بنجاح لـ ${tokens.length} جهاز في نفس اللحظة! 🔥`);
        setNotifyTitle("");
        setNotifyBody("");
      } else {
        alert("❌ لسه معملناش ملف الـ API.. هنجهزه في الخطوة الجاية!");
      }
    } catch (error) {
      console.error(error);
          ...
          <TabsContent value="notify">
            <Card className="border-yellow-400 bg-gradient-to-br from-[#1e2a4a] to-[#13213a] shadow-2xl">
              <CardHeader className="text-center pb-2">
                ...
                <p className="text-cyan-300 mt-2">الإشعار هيوصل لكل الموبايلات المشتركة في نفس اللحظة برنة!</p>
              </CardHeader>
              <CardContent className="p-6 md:p-10 space-y-6 max-w-3xl mx-auto">
                <div>
                  <label className="block text-sm font-bold text-gray-400 mb-2">عنوان الإشعار (قصير وجذاب)</label>
                  <Input 
                    value={notifyTitle} 
                    onChange={e => setNotifyTitle(e.target.value)} 
                    placeholder="مثال: ⚽ جووووول قاتل للنجيلة!" 
                    className="text-xl bg-[#0a1428] border-yellow-400/50 text-white p-6 font-bold" 
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-400 mb-2">تفاصيل الإشعار (محتوى الخبر)</label>
                  <Input 
                    value={notifyBody} 
                    onChange={e => setNotifyBody(e.target.value)} 
                    placeholder="مثال: النجيلة يسجل الهدف الأول في شباك علوش في الدقائق الأخيرة من المباراة." 
                    className="text-lg bg-[#0a1428] border-yellow-400/50 text-white p-6" 
                  />
                </div>
                <Button 
                  onClick={sendNotification} 
                  disabled={isSending}
                  className="w-full bg-yellow-400 text-black py-8 font-black text-2xl hover:bg-yellow-500 hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(250,204,21,0.3)] mt-8"
                >
