...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (!isAuth) return;
    const unsubMatches = onSnapshot(collection(db, "matches"), (snap) => setMatches(snap.docs.map(d => ({ id: d.id, ...d.data(), teamA: cleanTeamString(d.data().teamA), teamB: cleanTeamString(d.data().teamB) }))));
  ...
  const addOrUpdateGoal = async () => {
    ...
    if (editingGoalId) { 
      ...
      if (existingPlayer) {
        ...
        await updateDoc(doc(db, "goals", existingPlayer.id), updateData); alert(`✅ تم التحديث تراكمياً`);
      } else {
        const data = { player: playerNameTrimmed, team: teamSelected, goals: goalsToAdd, imageUrl: imageToAdd };
  ...
  const deleteAllPredictions = async () => {
     ...
     predictions.forEach(async (p) => await deleteDoc(doc(db, "predictions", p.id)));
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <Card className="border-yellow-400 bg-[#13213a] mb-8 mt-4 shadow-2xl">
          ...
          <CardContent className="space-y-6 p-6 text-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <select value={matchForm.round} onChange={e => setMatchForm(p => ({...p, round: e.target.value}))} className="bg-[#1e2a4a] border border-yellow-400 rounded-2xl p-4 text-white font-bold">{["الجولة الأولى","الجولة الثانية","الجولة الثالثة","الجولة الرابعة","الملحق","دور الستة عشر","دور الثمانية","نصف النهائي","النهائي"].map(r => <option key={r} value={r}>{r}</option>)}</select>
              <Input type="time" value={matchForm.time} onChange={e => setMatchForm(p => ({...p, time: e.target.value}))} className="text-white bg-[#1e2a4a] border-yellow-400 rounded-2xl p-4 font-bold" />
        ...
        <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)}>
          ...
          <TabsContent value="knockout">
            <div className="space-y-10">
              <div className="bg-gradient-to-br from-[#1e2a4a] to-[#13213a] p-4 sm:p-6 rounded-3xl border-2 border-yellow-400/50 shadow-xl relative overflow-hidden">
                 <div className="text-center mb-8"><Badge className="bg-yellow-400 text-black text-xl px-10 py-2 font-black shadow-lg border-2 border-[#13213a]">دور الـ 16</Badge></div>
                   ...
                   <div className="space-y-4">
                     {[ { id: 1, r1: 1, l1: "أول الترتيب", m2: 104 }, { id: 2, r1: 8, l1: "ثامن الترتيب", m2: 97 }, { id: 3, r1: 4, l1: "رابع الترتيب", m2: 101 }, { id: 4, r1: 5, l1: "خامس الترتيب", m2: 100 } ].map(match => (
                       <div key={match.id} className="bg-[#0a1428] rounded-2xl border border-yellow-400/30 flex items-center shadow-md overflow-hidden hover:border-yellow-400 transition-colors">
                          ...
                          <div className="flex-1 p-3 flex flex-col"><div className="text-emerald-400 font-bold text-sm text-center bg-[#13213a] rounded-lg py-1 mb-1">{standings.length >= match.r1 ? standings[match.r1-1].team : match.l1}</div><div className="text-center text-yellow-500 font-black text-xs my-1">VS</div><div className="text-cyan-300 font-bold text-sm text-center bg-[#13213a] rounded-lg py-1 mt-1">الفائز من م {match.m2}</div></div>
                       </div>
                     ))}
                   </div>
                   <div className="space-y-4">
              ...
              <div className="bg-[#13213a] p-4 sm:p-6 rounded-3xl border border-sky-500/30 shadow-lg">
                 ...
                 <div className="grid md:grid-cols-2 gap-4">
                   {[ { id: 97, r1: 9, r2: 24, l1: "تاسع الترتيب", l2: "الرابع والعشرون" }, { id: 98, r1: 10, r2: 23, l1: "عاشر الترتيب", l2: "الثالث والعشرون" }, { id: 99, r1: 11, r2: 22, l1: "الحادي عشر", l2: "الثاني والعشرون" }, { id: 100, r1: 12, r2: 21, l1: "الثاني عشر", l2: "الواحد والعشرون" }, { id: 101, r1: 13, r2: 20, l1: "الثالث عشر", l2: "العشرون" }, { id: 102, r1: 14, r2: 19, l1: "الرابع عشر", l2: "التاسع عشر" }, { id: 103, r1: 15, r2: 18, l1: "الخامس عشر", l2: "الثامن عشر" }, { id: 104, r1: 16, r2: 17, l1: "السادس عشر", l2: "السابع عشر" } ].map(match => (
                     <div key={match.id} className="bg-[#1e2a4a] rounded-xl flex items-center justify-between p-2 border border-white/5 hover:border-sky-400/50 transition-colors"><div className="flex-1 text-center font-bold text-white text-sm">{standings.length >= match.r1 ? standings[match.r1-1].team : match.l1}</div><div className="bg-[#0a1428] border border-sky-500/30 px-3 py-1 rounded-lg text-sky-400 font-black text-xs mx-2">م {match.id}</div><div className="flex-1 text-center font-bold text-gray-300 text-sm">{standings.length >= match.r2 ? standings[match.r2-1].team : match.l2}</div></div>
                   ))}
                 </div>
              </div>

              <div className="mt-12 border-t border-yellow-400/20 pt-10">
                <h3 className="text-2xl font-black text-center text-yellow-300 mb-8">إدارة مباريات الإقصاء</h3>
                        ...
                        {roundMatches.map(match => (
                          <div key={match.id} className="bg-[#1e2a4a] p-4 rounded-2xl flex flex-col justify-between gap-4 border border-yellow-400/30 shadow-md">
                            <div>
                               ...
                               <div className="text-cyan-300 text-center mt-2 text-sm font-bold">{match.date} • {match.status || match.time}</div>
                            </div>
          ...
          <TabsContent value="predictions">
            <Card className="border-yellow-400 bg-[#13213a]">
              <CardHeader className="flex flex-col sm:flex-row justify-between items-center gap-4 border-b border-white/10 pb-4">
                <CardTitle className="text-yellow-300 flex items-center gap-2"><Gift /> مسابقة التوقعات للجماهير</CardTitle>
          ...
          <TabsContent value="live" className="space-y-6">
            {matches.filter(m => m.isLive).length === 0 && (<Card className="border-yellow-400 bg-[#13213a] border-dashed"><CardContent className="p-16 text-center"><p className="text-xl text-cyan-300 mb-4">لا توجد مباريات جارية الآن</p></CardContent></Card>)}
