...
import { Trophy, LogOut, Edit, Trash2, Plus, Minus, Play, Pause, BellRing } from "lucide-react";
...
export default function AdminPage() {
  ...
  // 🔴 ضفنا الـ status عشان نتحكم في المباراة إنها انتهت
  ...
  useEffect(() => {
    ...
    const unsubMatches = onSnapshot(collection(db, "matches"), (snap) => {
      setMatches(snap.docs.map(d => ({ id: d.id, ...d.data(), teamA: cleanTeamString(d.data().teamA), teamB: cleanTeamString(d.data().teamB) })));
    });
    const unsubGoals = onSnapshot(collection(db, "goals"), (snap) => setGoals(snap.docs.map(d => ({ id: d.id, ...d.data(), team: cleanTeamString(d.data().team) }))));
  ...
  const addOrUpdateGoal = async () => {
    ...
    if (editingGoalId) { 
      ...
      await updateDoc(doc(db, "goals", editingGoalId), data); setEditingGoalId(null); alert("✅ تم تعديل الهدف بنجاح");
    } else { 
      ...
      if (existingPlayer) {
        ...
        await updateDoc(doc(db, "goals", existingPlayer.id), updateData); alert(`✅ تم التحديث تراكمياً.. رصيد اللاعب أصبح (${newTotalGoals}) أهداف`);
      } else {
        const data = { player: playerNameTrimmed, team: teamSelected, goals: goalsToAdd, imageUrl: imageToAdd };
        await addDoc(collection(db, "goals"), data); alert("✅ تم إضافة اللاعب والهدف بنجاح");
      }
    }
  ...
  // 🔴 الحل التراكمي للكروت عشان اللاعب ميختفيش
  const addCard = async () => {
    ...
    const existingPlayer = cardEvents.find(c => c.player.trim().toLowerCase() === playerNameTrimmed.toLowerCase() && c.team === teamSelected);
    ...
    if (existingPlayer) {
       ...
       await updateDoc(doc(db, "cards", existingPlayer.id), { yellow: newYellow, red: newRed });
       alert(`✅ تم تحديث كروت اللاعب بنجاح!`);
    } else {
       await addDoc(collection(db, "cards"), { player: playerNameTrimmed, team: teamSelected, yellow: cardForm.type === "yellow" ? 1 : 0, red: cardForm.type === "red" ? 1 : 0 });
  ...
  const sendNotification = async () => {
    if (!notifyTitle || !notifyBody) return alert("اكتب عنوان وتفاصيل الإشعار أولاً يا بطل!");
    setIsSending(true);
    try {
      ...
      if (res.ok) { alert(`✅ تم إرسال الإشعار بنجاح لـ ${tokens.length} جهاز في نفس اللحظة! 🔥`); setNotifyTitle(""); setNotifyBody(""); } 
      else { alert("❌ حدث خطأ في ملف السيرفر."); }
    ...
    <div dir="rtl" className="min-h-screen bg-[#0a1428] text-white">
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)}>
          <TabsList className="grid grid-cols-4 md:grid-cols-8 bg-[#13213a] border border-yellow-400/50 p-1.5 rounded-2xl mb-8 flex-wrap h-auto">
            ...
            <TabsTrigger value="notify" className="data-[state=active]:bg-yellow-400 data-[state=active]:text-black font-black py-3 rounded-xl text-yellow-400 bg-black/40 border border-yellow-400/20">إشعارات 🔔</TabsTrigger>
          </TabsList>

          <TabsContent value="notify">
            <Card className="border-yellow-400 bg-gradient-to-br from-[#1e2a4a] to-[#13213a] shadow-2xl">
              <CardHeader className="text-center pb-2">
                <BellRing className="mx-auto h-12 w-12 text-yellow-400 mb-4 animate-bounce" />
                <CardTitle className="text-3xl font-black text-yellow-300">إرسال إشعار عاجل للجمهور 🚀</CardTitle>
              </CardHeader>
          ...
          <Card className="border-yellow-400 bg-[#13213a] mb-8 mt-8">
            ...
            <CardContent className="space-y-6 p-6 text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                {/* 🔴 ده الزرار الجديد اللي هيحللك مشكلة ماتشات النهاردة */}
                <select value={matchForm.status} onChange={e => setMatchForm(p => ({...p, status: e.target.value}))} className="bg-[#1e2a4a] border border-red-500 rounded-2xl p-4 text-white font-bold">
                  <option value="لم تبدأ">حالة المباراة: لم تبدأ</option>
                  <option value="انتهت">حالة المباراة: انتهت ✔️</option>
                </select>
              </div>
