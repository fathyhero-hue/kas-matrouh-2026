...
import { Loader2, Calendar, Clock, Trophy, Target, Shield, ShieldAlert, Zap, BellRing, Play, Gift } from "lucide-react";
...
// 🔴 دالة الترتيب مُضاف ليها خصم الـ 3 نقاط إدارياً 🔴
...
export default function Page() {
  const [matches, setMatches] = useState<any[]>(INITIAL_MATCHES.map((m: any) => ({
    ...m, home: cleanTeamString(m.home || m.teamA), away: cleanTeamString(m.away || m.teamB),
    teamA: cleanTeamString(m.teamA || m.home), teamB: cleanTeamString(m.teamB || m.away)
  })));
  ...
  const [activeTab, setActiveTab] = useState<"live" | "today" | "tomorrow" | "all" | "standings" | "scorers" | "stats" | "cards" | "media">("standings");
  const [isSubscribed, setIsSubscribed] = useState(false);
  
  const [predForms, setPredForms] = useState<Record<string, {name: string, phone: string, home: string, away: string}>>({});
  ...
  useEffect(() => {
    ...
    const unsubMatches = onSnapshot(collection(db, "matches"), (snap) => {
      setMatches(snap.docs.map((doc) => {
        const d = doc.data();
        return {
          id: doc.id, home: cleanTeamString(d.teamA || d.home), away: cleanTeamString(d.teamB || d.away),
          teamA: cleanTeamString(d.teamA || d.home), teamB: cleanTeamString(d.teamB || d.away),
          ...
          motm: d.motm || "", motmImage: d.motmImage || ""
        };
      }));
      setLoading(false);
    }, () => setLoading(false));
    ...
    return () => { unsubMatches(); unsubGoals(); unsubCards(); unsubMedia(); unsubTicker(); };
  }, []);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (permission === "granted") {
        ...
        if (!supported) { alert("متصفحك الحالي لا يدعم الإشعارات."); return; }
        ...
        if (token) {
          ...
          setIsSubscribed(true); alert("✅ تم تفعيل الإشعارات بنجاح!");
        }
      }
    } catch (error: any) { alert("حدث خطأ: " + (error.message || "تأكد من الاتصال بالإنترنت.")); }
  };

  const submitPrediction = async (match: any) => {
    const form = predForms[match.id];
    if (!form || !form.name || !form.phone || form.home === undefined || form.away === undefined || form.home === "" || form.away === "") {
        return alert("الرجاء إدخال الاسم ورقم الهاتف وتوقع الأهداف كاملة!");
    }
    try {
        await addDoc(collection(db, "predictions"), {
            matchId: match.id, matchName: `${match.teamA} ضد ${match.teamB}`,
            name: form.name.trim(), phone: form.phone.trim(), homeScore: form.home, awayScore: form.away,
            timestamp: new Date().toISOString()
        });
        const newPredicted = { ...predictedMatches, [match.id]: true };
        setPredictedMatches(newPredicted);
        localStorage.setItem('predictedMatches', JSON.stringify(newPredicted));
        alert("✅ تم إرسال توقعك بنجاح! سيتم التواصل مع الفائزين.");
    } catch (error) { console.error(error); alert("حدث خطأ أثناء الإرسال"); }
  };
        ...
        {!predictedMatches[match.id] ? (
          <div className="space-y-3 bg-[#0a1428] p-4 rounded-2xl border border-yellow-400/20 shadow-inner">
            <div className="grid grid-cols-2 gap-3">
               ...
               <Input placeholder="رقم الهاتف (للتواصل)" type="tel" className="bg-[#1e2a4a] border-none text-white text-xs h-10 font-mono" onChange={(e) => setPredForms({...predForms, [match.id]: {...predForms[match.id], phone: e.target.value}})} />
            </div>
            <div className="flex items-center justify-center gap-4 bg-[#13213a] p-3 rounded-xl border border-white/5">
               <div className="text-xs text-cyan-300 font-bold">{match.teamA}</div>
               ...
               <div className="text-xs text-cyan-300 font-bold">{match.teamB}</div>
            </div>
    ...
    <div dir="rtl" className="min-h-screen bg-[#0a1428] text-white relative pb-20">
      <div className="mx-auto max-w-7xl p-3 sm:p-4 md:p-6 lg:p-8">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {[
            { key: "live", label: "مباشر", icon: "🔴" }, { key: "stats", label: "إحصائيات", icon: "📊" }, { key: "today", label: "اليوم", icon: "📅" }, 
            { key: "tomorrow", label: "غداً", icon: "📆" }, { key: "all", label: "النتائج", icon: "⚽" }, { key: "standings", label: "الترتيب", icon: "📊" }, 
            { key: "scorers", label: "الهدافين", icon: "🥇" }, { key: "cards", label: "الكروت", icon: "🟨" }, { key: "media", label: "ميديا ونجوم", icon: "🌟" }
          ].map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key as any)} className={`flex-1 sm:flex-none px-5 py-3.5 rounded-2xl font-bold text-sm transition-all border border-yellow-400/30 ${activeTab === tab.key ? "bg-yellow-400 text-black shadow-lg scale-105" : "bg-[#1e2a4a] text-white hover:bg-[#25345a]"}`}>
        ...
        {activeTab === "media" && (
          <div className="space-y-12">
            <div>
               <h2 className="text-3xl font-black text-yellow-400 mb-6 flex items-center gap-2"><Trophy /> نجوم المباريات</h2>
               {finishedMatches.filter(m => m.motm).length > 0 ? (
                 <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                   {finishedMatches.filter(m => m.motm).map(m => (
                      <Card key={m.id} className="bg-gradient-to-br from-[#1e2a4a] to-[#13213a] border-yellow-400/50 rounded-3xl overflow-hidden shadow-lg relative transition-transform hover:scale-[1.02]">
                        <div className="absolute top-0 right-0 bg-yellow-400 text-black font-black px-4 py-1 rounded-bl-xl z-10 text-sm shadow-md">رجل المباراة 🌟</div>
                        <div className="p-6 text-center">
                          <div className="h-24 w-24 mx-auto rounded-full border-4 border-yellow-400 overflow-hidden mb-4 bg-[#0a1428] flex items-center justify-center shadow-lg">
                            {m.motmImage ? <img src={m.motmImage} className="h-full w-full object-cover" /> : <span className="text-4xl">👤</span>}
                          </div>
                          <h3 className="text-2xl font-black text-white mb-1">{m.motm}</h3>
                          <p className="text-cyan-300 font-bold mb-4">{m.teamA} vs {m.teamB}</p>
                          <Badge className="bg-black/50 text-yellow-300 border border-yellow-400/30 text-lg px-4 py-1">{m.homeGoals} - {m.awayGoals}</Badge>
                        </div>
                      </Card>
                   ))}
                 </div>
               ) : <p className="text-center text-gray-500 py-10">لم يتم اختيار نجوم للمباريات حتى الآن</p>}
            </div>
            <div>
               ...
               ) : <p className="text-center text-gray-500 py-10">لا توجد فيديوهات حالياً</p>}
            </div>
          </div>
        )}
        ...
        {activeTab === "live" && (
          <Card className="rounded-3xl border-2 border-red-500/50 bg-[#13213a] shadow-[0_0_20px_rgba(239,68,68,0.2)]">
            ...
            <CardContent className="p-6 grid gap-6">
                ...
                <div key={match.id} className="relative rounded-3xl bg-gradient-to-r from-[#1e2a4a] to-[#25345a] border border-white/10 p-8 overflow-hidden">
                  ...
                  <div className="mt-6 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex-1 text-center md:text-right">
                      <div className="flex items-center justify-center md:justify-end gap-2 mb-2"><div className="text-2xl font-bold text-white">{match.home}</div>{match.redCardsHome > 0 && Array.from({length: match.redCardsHome}).map((_, i) => <div key={i} className="w-4 h-5 bg-red-600 rounded-sm border border-white/50 shadow-sm" title="طرد"></div>)}</div>
                    </div>
              ...
              )}) : <div className="py-20 text-center"><Calendar className="h-16 w-16 text-gray-600 mx-auto mb-4 opacity-20" /><p className="text-xl text-gray-500">لا توجد مباريات جارية في الوقت الحالي</p></div>}
            </CardContent>
          </Card>
        )}
